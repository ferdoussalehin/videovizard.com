...
<head>
    ...
    <style>
		...
		/* Fix for visibility in inputs and dropdowns */
		...
		/* Ensure the language picker looks correct */
		#langPicker {
			appearance: auto; /* Ensures the arrow is visible */
			padding: 8px;
		}
		#scriptInput {
		...
		/* Controls the horizontal gap between letters */
		letter-spacing: 0.5px; 
		
		/* Ensures the font looks clean */
...
THE END.</textarea>
                    ...
                    <div class="grid grid-cols-2 gap-2">
					 ...
					 <div class="border-t border-slate-800 pt-4 mt-4">
						<input type="hidden" id="rowId" value="123"> <!-- replace 123 with your reel DB row ID -->
					 
						<button id="generateAudioBtn" class="bg-emerald-500 text-white px-4 py-2 rounded-xl mt-2">
							🎙 Generate Audio 
						</button>

						<p id="audioStatus" class="mt-2 text-sm text-white"></p>

						<audio id="voicePreview" controls style="display:none;" class="mt-2 w-full"></audio>
						<p id="audioStatus" class="mt-2 text-sm text-white"></p>
					 
					 
						<div class="music-upload">
							<label for="bgMusic">Upload Background Music (MP3)</label>
							<input type="file" id="bgMusic" accept="audio/mp3,audio/mpeg" />
       ...
       <div class="lg:col-span-4 space-y-4">
			<div class="glass">
				<label class="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-4">Post-Processing</label>
				<div class="space-y-4">
					<div>
						<div class="flex justify-between mb-1">
							<label class="text-[10px] font-bold text-slate-300 uppercase">Exposure</label>
							<span id="brightVal" class="text-[10px] text-emerald-500">100%</span>
						</div>
						<input type="range" id="brightCtrl" min="0" max="200" value="100">
					</div>
					<div>
						<div class="flex justify-between mb-1">
							<label class="text-[10px] font-bold text-slate-300 uppercase">Depth Blur</label>
							<span id="blurVal" class="text-[10px] text-emerald-500">0px</span>
						</div>
						<input type="range" id="blurCtrl" min="0" max="25" value="0">
					</div>
				</div>
			</div>
			...
			<div class="glass">
				...
				<div class="space-y-3">
					<select id="langPicker" class="w-full">
						<option value="contents_en">English (Default)</option>
					...
					<div class="flex gap-2">
						<button id="fetchDbBtn" type="button" class="flex-1 bg-slate-800 hover:bg-slate-700 text-[10px] font-bold py-2 rounded-lg border border-slate-700 text-white uppercase transition-all">
							📥 Load Row
						</button>
						<button id="markSocialDone" onclick="updateSocialStatus()" style="background: #4f46e5; color: white; padding: 10px; border-radius: 5px; cursor: pointer; margin-left: 10px;">
							📱 Mark Social Done
						</button>
					</div>
					<div id="dbInfo" class="text-[9px] text-slate-500 font-mono text-center">Ready</div>
				</div>
			</div>
    ...
    <script>
	  ...
	  { url: 'https://assets.mixkit.co/videos/preview/mixkit-smoke-background-1184-large.mp4' }
        ...
        let state = { 
            ...
            bg: imgLibrary[0].url, speed: 0.5, fontSize: 30, color: '#ffffff', mode: 'scroll',
            fontFamily: "'Inter', sans-serif", fontStyleClasses: "italic font-black",
            brightness: 100, blur: 0, posY: 568, isRecording: false, 
            isVideo: false, currentMode: 'img',lineHeight: 1.5,
        ...
        function drawToCanvas() {
            ...
            const lineHeight = (state.fontSize * 1.5) * scale;
            let startY;
            if (state.mode === 'scroll') {
        ...
        document.getElementById('exportBtn').onclick = async () => 
		{
			...
			// OPTIONAL: Add uploaded music
			if (uploadedMusicURL) {
				const music = new Audio(uploadedMusicURL);
				music.loop = true;
				music.play();
            ...
            // 1. Determine the best supported format
			let supportedMime = 'video/webm'; // Default fallback
			if (MediaRecorder.isTypeSupported('video/webm;codecs=h264')) {
				supportedMime = 'video/webm;codecs=h264';
			...
			try {
				recorder = new MediaRecorder(finalStream, { mimeType: supportedMime });
			} catch (e) {
				console.error("MimeType not supported, falling back to default");
				recorder = new MediaRecorder(stream); // Ultimate fallback to fix the "no-click" issue
			}
			...
			recorder.onstop = () => {
				// We package it as an MP4 container regardless of the internal codec
				const blob = new Blob(chunks, { type: 'video/mp4' });
				uploadVideoToServer(blob); // we'll send this to server
				const url = URL.createObjectURL(blob);
				const a = document.createElement('a');
				a.href = url; 
				a.download = `broll-export-${Date.now()}.mp4`;
		...
		function uploadVideoToServer(blob) {
			...
			formData.append('video', blob, `broll-export-${Date.now()}.mp4`);

			fetch('upload_video.php', { // your server endpoint
				method: 'POST',
				body: formData
			})
			...
			.then(data => {
				...
				alert('Video saved on server: ' + data.file);
			})
			.catch(error => {
		...
		// Variable to store the full row data from the DB
		let currentDbRow = null;

				// 1. FUNCTION TO FETCH FROM DB
		async function fetchFromDB() {
			// 1. Get the raw value from the picker (e.g., "contents_en")
			const rawLang = document.getElementById('langPicker').value;
			
			const inputField = document.getElementById('scriptInput');
			
			// 2. Extract just the 'en' or 'ur' part (the last 2 characters)
			const langCode = rawLang.split('_').pop(); // Takes "en" from "contents_en"
			...
			try {
				...
				if (data.main) {
					...
					// 3. Map to your specific Database Keys
					// Main uses the full rawLang (contents_en)
					// CTA uses the short langCode (cta_en)
					const mainText = data.main[rawLang] || "";
				
					
					
					const ctaKey = `cta_${langCode}`;
					
					const ctaText = data.cta[ctaKey] || "";

					const finalScript = (mainText + "\n\n" + ctaText).trim();

					// 4. Inject into UI
					if (inputField) {
						inputField.value = finalScript;
						
						// Update teleprompter state
						state.text = finalScript;
						state.words = finalScript.split(/\s+/);
						
						if (typeof update === "function") update();
						console.log("Success! Used Key:", rawLang, "and", ctaKey);
					}
				}
			} catch (e) {
		...
		// Update the language picker to use the same 's' logic
		document.getElementById('langPicker').onchange = function() {
			if (currentDbRow && currentDbRow.main) {
				const lang = this.value;
				const mainText = currentDbRow.main[`contents_${lang}`] || "";
				const ctaText = currentDbRow[`cta_${lang}`] || "";
				
				const finalScript = (mainText + "\n\n" + ctaText).trim();
				document.getElementById('scriptInput').value = finalScript;
				state.text = finalScript;
				state.words = finalScript.split(/\s+/);
				if (typeof update === "function") update();
			}
		...
		musicInput.addEventListener("change", function (event) {
			...
			// Show file name
			musicName.textContent = "Selected: " + file.name;

			// Show audio preview
			musicPreview.src = uploadedMusicURL;
		...
		function playGeneratedVoice() {
			...
			if (!preview) {
				console.error("voicePreview element not found");
				return;
			}

			// Hardcoded filename for now
			const fileName = "socialmedia_audios/voice_123_contents_en.mp3";

			// Set source and show player
			preview.src = fileName + "?t=" + new Date().getTime(); // prevent caching
			...
			preview.play()
				.then(() => {
					status.innerText = "▶ Playing generated voice audio!";
				})
				.catch(err => {
					console.warn("Autoplay blocked:", err);
					// Fallback: user must click play
					status.innerHTML = '✅ Audio generated! <strong>Click ▶ to play.</strong>';
				});
		}
		...
		document.getElementById("generateAudioBtn").onclick = async function() {
			// Ensure a record is fetched
			/*
			if (!currentDbRow || !currentDbRow.id) {
				alert("Please fetch a record first!");
				return;
			}

			const rowId = currentDbRow.id; // dynamic row ID
			
			*/
			const rowId = 123;
			const text = document.getElementById("scriptInput").value.trim();
			const langCode = document.getElementById("langPicker").value;
			...
			try {
				...
				// Get raw response text first (avoid JSON errors)
				const textResult = await response.text();
				console.log("Server raw response:", textResult);
				...
				if (result.success) {
					...
					playGeneratedVoice(rowId, langCode);
		...
		async function updateSocialStatus() {
			// Check if we actually have a record loaded
			if (!currentDbRow || !currentDbRow.main) {
				alert("Please load a record first!");
				return;
			}

			const recordId = currentDbRow.main.id; // Get ID from the loaded record

			try {
				// We call the PHP with a specific action for the social table
				const response = await fetch(`fetch_record.php?action=update_social&id=${recordId}&status=posted`);
				const result = await response.json();

				if (result.success) {
					alert("Social Media table updated to 'posted'!");
					
					// UI Feedback: Change button color to show it's done
					document.getElementById('markSocialDone').style.background = "#9ca3af";
					document.getElementById('markSocialDone').innerText = "✅ Posted";
				} else {
		...
		document.addEventListener("DOMContentLoaded", function() {
			...
			btn.addEventListener("click", async function() {
				...
				// Dynamically get row ID from your JS context
				// Example: assume each reel has a data attribute
				// <div id="currentReel" data-row-id="123"></div>
				const rowId = document.getElementById("currentReel").dataset.rowId;
				...
				try {
					...
					if (result.success) {
						preview.src = result.file + "?t=" + new Date().getTime();
						preview.style.display = "block";
						preview.load();
						preview.play();
						status.innerText = "✅ Audio generated successfully!";
		...
		// 3. LISTEN FOR LANGUAGE SWITCH
		// If you change the language dropdown, it swaps the text of the CURRENTLY loaded row
		

		// 4. ATTACH CLICK TO BUTTON
		document.getElementById('fetchDbBtn').onclick = fetchFromDB;
        window.onload = init;
    </script>
</body>
</html>
