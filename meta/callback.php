<?php
/**
 * callback_patched.php  (drop-in replacement for meta/callback.php)
 * Placed in: meta/callback_patched.php
 *
 * DEPLOY INSTRUCTIONS:
 *   1. Rename the original:  callback.php  →  callback_original.php
 *   2. Rename this file:     callback_patched.php  →  callback.php
 *
 * What changed vs the original callback.php:
 *   • After writing tokens.json, checks $_SESSION['fb_oauth_return'].
 *   • If that session key is set, redirects to callback_vizard.php
 *     (which saves tokens to DB then goes back to the scheduler).
 *   • If not set, falls through to the original HTML "Setup Complete" page
 *     exactly as before — zero behaviour change for the legacy flow.
 *
 * Everything else is 100% identical to the original callback.php.
 */
...
// ═══════════════════════════════════════════════════════════════════════════════
// PATCH: If called from the VideoVizard scheduler flow, redirect to
//        callback_vizard.php which saves tokens to the DB and returns to
//        the scheduler. This is the only change vs the original callback.php.
// ═══════════════════════════════════════════════════════════════════════════════
if (!empty($_SESSION['fb_oauth_return'])) {
    header('Location: callback_vizard.php');
    exit;
}
// ═══════════════════════════════════════════════════════════════════════════════
// END PATCH — everything below is the original "Setup Complete" HTML page.
// ═══════════════════════════════════════════════════════════════════════════════
