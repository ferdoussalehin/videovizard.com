...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'get_promo_categories') {
    ...
    if ($res) while ($row = mysqli_fetch_assoc($res)) {
        ...
        if (!empty($row['sub_categories'])) {
            $decoded = json_decode($row['sub_categories'], true);
            if (is_array($decoded)) $subs = $decoded;
        }
        // Fix broken charset in old rows
        $icon = mb_convert_encoding($row['category_icon'], 'UTF-8', 'UTF-8');
        $name = mb_convert_encoding($row['category_name'], 'UTF-8', 'UTF-8');
        $cats[] = [
            'category_key'              => $row['category_key'],
            'category_name'             => $name,
            'category_icon'             => $icon,
            'sub_categories'            => $subs,
            'has_product_name'          => (bool)($row['has_product_name'] ?? true),
            'product_name_label'        => $row['product_name_label'] ?? 'Product Name',
            'product_name_placeholder'  => $row['product_name_placeholder'] ?? '',
            'needs_image'               => (bool)($row['needs_image'] ?? false),
            'needs_model'               => (bool)($row['needs_model'] ?? false),
        ];
    }
    $json = json_encode(['success' => true, 'categories' => $cats], JSON_UNESCAPED_UNICODE);
    if ($json === false) {
        // Try again stripping invalid bytes
        array_walk_recursive($cats, function(&$v) {
            if (is_string($v)) $v = mb_convert_encoding($v, 'UTF-8', 'UTF-8');
        });
        $json = json_encode(['success' => true, 'categories' => $cats], JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE);
    }
    echo $json;
    exit;
}
...
// ══════════════════════════════════════════════════════════
// ── Load subcategories from hdb_promo_subcategories ─────────
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'get_subcategories') {
    $grp = mysqli_real_escape_string($conn, trim($_POST['promo_group'] ?? ''));
    $rows = [];
    if ($grp) {
        $r = mysqli_query($conn,
            "SELECT promo_subgroup FROM hdb_promo_subcategories
             WHERE promo_group='$grp' AND is_active=1
             ORDER BY display_order ASC, promo_subgroup ASC");
        while ($row = mysqli_fetch_assoc($r)) $rows[] = $row['promo_subgroup'];
    }
...
async function loadCategories() {
    ...
    try {
        ...
        try { d = JSON.parse(txt); }
            ...
            console.error('[loadCategories] Bad JSON response:', txt.substring(0, 500));
            document.getElementById('cat-grid').innerHTML = '<div style="color:red;padding:16px;">⚠ Failed to load categories — see console for details</div>';
            return;
        }
    } catch(e) {
        console.error('[loadCategories] Fetch error:', e.message);
        return;
    }
...
function selectCategory(tile, cat, silent = false) {
    ...
    // Sub-category chips — load from hdb_promo_subcategories
    const subCard = document.getElementById('sub-card');
    const chips   = document.getElementById('sub-chips');
    chips.innerHTML = '<span style="color:var(--muted);font-size:12px;">Loading…</span>';
    subCard.style.display = '';

    const scFd = new FormData();
    scFd.append('ajax_action', 'get_subcategories');
    scFd.append('promo_group', S.cat_name);
    fetch('', { method:'POST', body:scFd }).then(r=>r.json()).then(d => {
        chips.innerHTML = '';
        const subs = d.subcategories || S.sub_cats || [];
        if (subs.length > 0) {
            subCard.style.display = '';
            subs.forEach((s, i) => {
                ...
                c.textContent = s;
                if (i === 0) { S.sub_category = s; }
                c.addEventListener('click', () => {
                    document.querySelectorAll('#sub-chips .chip').forEach(x => x.classList.remove('active'));
    ...
    }).catch(() => { subCard.style.display = 'none'; });
...
async function startPromoVideo() {
    ...
    const AUDIO_TIMEOUT = 45000; // 45s per attempt
    ...
    if (S2.track === 'ai') {
        ...
        if (audioFailedAI.length > 0) {
            s2Log('↺ ' + audioFailedAI.length + ' audio(s) timed out — retrying now (45s each)…', 'warning');
            for (const scene of audioFailedAI) {
                ...
                try {
                    s2Log('↺ Retrying Scene '+(i+1)+' audio…', 'info');
                    const fp = fetch(S2_EP,{method:'POST',body:rFd,credentials:'include'}).then(r=>r.json());
                    const tp = new Promise((_,rej)=>setTimeout(()=>rej(new Error('timeout')),45000));
                    const d  = await Promise.race([fp,tp]);
                    if (d.success) { audioDone++; audioResults[i]=true; updateSceneBox(i,'audio'); s2Log('✓ Scene '+(i+1)+' audio retry OK ('+elapsed()+')', 'success'); }
                    else { audioFail++; s2Log('✗ Scene '+(i+1)+' audio retry failed: '+(d.error||'?'), 'error'); }
