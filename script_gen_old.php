...
if (isset($admin_id) && $admin_id > 0) {
    $admin_query = mysqli_query($conn, "SELECT name FROM admin_users WHERE id = '$admin_id' LIMIT 1");
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'generate_content_only') {
    ...
    if (empty($prompt)) {
        echo json_encode(['success' => false, 'message' => 'Prompt is required 1']);
        exit;
    }
...
The contents must be for only 30 seconds. if contents are too long, the reduce it.
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'process_free_content') {
    ...
    if (empty($prompt)) {
        echo json_encode(['success' => false, 'message' => 'Prompt is required 2']);
        exit;
    }
...
<head>
    <meta charset="UTF-8">
    <title>VideoVizard - 1. Create script</title>
    <style>
		...
		.tab {
			...
			background: #e0f2fe; /* Light blue background for unselected tabs */
			transition: all 0.2s;
		}
		.tab:hover {
		...
		.tab.active {
			background: #0f2a44; /* Dark blue for active tab */
			color: white;
		}
		.tab-content {
			display: block;
...
.modal-msg.success { color: #059669; }
.modal-msg.error { color: #dc2626; }
/* Button color matching for Free Format Creator */
.mode-btn#btn_ai_mode.active {
    background: linear-gradient(90deg, #4f46e5, #6366f1) !important;
}

/* Ensure the title box uses the same color as the AI button */
.mode-title-box.ai-mode {
    background: linear-gradient(90deg, #4f46e5, #6366f1) !important;
}

/* Update the button colors in the HTML */
#btn_ai_mode {
    background: linear-gradient(90deg, #4f46e5, #6366f1) !important;
}
...
.success-card h2 { color: #16a34a; margin-top: 0; }
.btn-primary { background: #2563eb; color: white; border: none; padding: 12px 20px; border-radius: 8px; cursor: pointer; font-weight: bold; width: 100%; margin-bottom: 10px; }
.btn-secondary { background: #f1f5f9; color: #475569; border: none; padding: 12px 20px; border-radius: 8px; cursor: pointer; width: 100%; }
...
.button-group {
    ...
    gap: 10px;
    margin-top: 10px;
    min-height: 48px; /* Fixed height for button container */
}
...
/* Make both buttons the same when one is hidden */
.button-group:has(#freeCreateScenesBtn[style*="display: none"]) #freeProcessBtn {
    width: 100%;
}
...
.btn-primary, .btn-success {
    padding: 10px 16px;  /* Same padding for both */
...
<div class="container">
    ...
    <h2 style="font-size: 14px; font-weight: normal; color: #888;">
		Generate a compelling video script in seconds using AI — or start from scratch and customize it your way.
	</h2>

    <div class="tab-bar">
        <div class="tab active" onclick="switchTab('free')" id="tab-free">✏️ Write from Scratch</div>
        <div class="tab" onclick="switchTab('database')" id="tab-database">📚 From stired database</div>
    </div>
	</div>
  ...
  <div class="card">
    <!-- Title box will be inserted here by JavaScript -->
    ...
    <div class="form-group" style="margin-bottom:20px;">
      ...
      <div style="display:flex; gap:10px; margin-top:6px;">
        <button type="button" id="btn_ai_mode" class="mode-btn active" onclick="setFreeMode('ai')"
          style="flex:1; padding:14px; font-size:16px; background:linear-gradient(90deg,#4f46e5,#6366f1); color:white; border:none; border-radius:10px; cursor:pointer;">
          ✨ My Idea (AI)
        </button>
        <button type="button" id="btn_user_mode" class="mode-btn" onclick="setFreeMode('user')"
          style="flex:1; padding:14px; font-size:16px; background:linear-gradient(90deg,#10b981,#34d399); color:white; border:none; border-radius:10px; cursor:pointer;">
          📝 My Content
        </button>
      </div>
    </div>
    ...
    <div id="free_ai_fields">
      <div class="form-group">
        ...
        <select id="free_reel_type">
          <option value="multi-scene" selected>📱 Dynamic Reel (Auto-cut into scenes with pauses)</option>
          <option value="single-scene">📺 Full Video (Single scene / B-Roll background)</option>
        </select>
      </div>
      ...
      <div class="form-group">
        <div style="display:flex; align-items:center; gap:10px; margin-bottom:4px;">
          ...
          <label for="free_auto_title" style="margin:0; font-weight:normal;">Auto-generate</label>
        </div>
      ...
      <!-- Audience with expanded options -->
      <div class="form-group">
        <label>👥 Who is this video for?</label>
        <select id="free_audience">
          <option value="general">General Audience</option>
          <option value="students">Students</option>
          <option value="social_media_manager">Social Media Managers</option>
          <option value="content_creators">Content Creators</option>
          <option value="engineers">Engineers</option>
          <option value="computer_professionals">Computer / IT Professionals</option>
      ...
      <div class="form-group">
        <div style="display: flex; align-items: center; gap: 4px; margin-bottom: 6px;">
          ...
          <label for="free_rnd_hook" style="margin:0;">Select hook randomly</label>
        </div>
        <select id="free_hook_select" style="width:100%; padding:10px; border-radius:8px; border:1px solid #cbd5e1;">
          ...
          <?php
			...
			if ($hooks_free && mysqli_num_rows($hooks_free) > 0) {
			  while ($hook = mysqli_fetch_assoc($hooks_free)) {
				...
				$hook_desc = htmlspecialchars($hook['hook_description']);
				$hook_label = htmlspecialchars($hook['hook_label']);
				echo "<option value='{$hook_name}'>{$hook_label}</option>";
			  }
			} else {
            echo "<option value='1'>Ever feel like...</option>";
          }
          ?>
        </select>
      </div>
    ...
    </div> <!-- Close free_ai_fields -->

    <!-- User Mode Fields -->
    <div id="free_user_fields" style="display:none;">
      <div class="form-group">
        <label>🎬 Reel Type</label>
        <select id="free_user_reel_type">
          <option value="multi-scene" selected>📱 Dynamic Reel (Auto-cut into scenes with pauses)</option>
          <option value="single-scene">📺 Full Video (Single scene / B-Roll background)</option>
        </select>
      </div>
      ...
      <div class="form-group">
        <label for="free_user_title">📌 Title</label>
        <input type="text" id="free_user_title" placeholder="Enter a title for your content..." 
               style="width:100%; padding:10px; border-radius:8px; border:1px solid #cbd5e1;">
      </div>
    ...
    </div> <!-- Close free_user_fields -->

    <!-- Processed Content - SHARED between both modes -->
    <div id="free_processed_container" style="display: none;">
  <div class="form-group">
    <label>📝 Processed Content (Edit if needed)</label>
    <textarea id="free_processed_content" rows="10" style="width:100%; padding:12px; border:2px solid #e2e8f0; border-radius:8px; font-family:monospace; font-size:14px;"></textarea>
  </div>
</div>

<!-- Buttons outside the hidden container so they're always visible -->
...
function createTitleBox() {
    const card = document.querySelector('#free-tab .card');
    const existingTitle = card.querySelector('h2');
    if (existingTitle) {
        existingTitle.style.display = 'none';
    }
    
    // Check if title box already exists
    let titleBox = document.querySelector('.mode-title-box');
    if (!titleBox) {
        titleBox = document.createElement('div');
        titleBox.className = 'mode-title-box ai-mode';
        titleBox.innerHTML = `
            <h2>
                ✨ Idea to video
        ...
        `;
        card.insertBefore(titleBox, card.firstChild);
    }
    return titleBox;
}
...
function setFreeMode(mode) {
    ...
    const titleBox = document.querySelector('.mode-title-box');
    if (titleBox) {
		titleBox.className = `mode-title-box ${mode}-mode`;
		const titleIcon = mode === 'ai' ? '✨' : '📝';
		const titleMain = mode === 'ai' ? 'Idea to Video' : 'Content to Video';
		const titleText = mode === 'ai' ? 'AI-Powered Creation' : 'My Content';
		titleBox.innerHTML = `
			<h2>
				${titleIcon} ${titleMain}
				<span class="mode-badge">${titleText}</span>
			</h2>
		`;
	}
}
...
function updateProgress(data) {
    ...
    const freeProcessBtn = document.getElementById('freeProcessBtn');
    const freeCreateScenesBtn = document.getElementById('freeCreateScenesBtn');
    ...
    // Auto-scroll to progress container when we get first update
    ...
    if (data.final && data.success) {
        ...
        if (createScenesBtn) {
            createScenesBtn.disabled = false;
            createScenesBtn.innerText = "🎬 Step 2: Create Scenes from Content";
        }
        if (generateContentBtn) {
            generateContentBtn.disabled = false;
            generateContentBtn.innerText = "📝 Step 1: Generate Content";
        }
        if (freeProcessBtn) {
            freeProcessBtn.disabled = false;
            freeProcessBtn.innerText = "📝 Step 1: Process Content";
        }
        if (freeCreateScenesBtn) {
            freeCreateScenesBtn.disabled = false;
            freeCreateScenesBtn.innerText = "🎬 Step 2: Create Scenes";
        }
        ...
        // Hide the progress container after showing modal
        setTimeout(() => {
            document.getElementById('progress_container').style.display = 'none';
        }, 500);

    } else if (data.final && !data.success) {
...
async function createFreeScenes() {
    ...
    const dbCreateBtn = document.getElementById('createScenesBtn');
    ...
    const mode = document.getElementById('btn_ai_mode').classList.contains('active') ? 'ai' : 'user';
    
    // Get title based on mode
    let title = '';
    if (mode === 'ai') {
        title = window.ai_freeTitle || window.freeTitle || '';
    } else {
        title = window.user_freeTitle || window.freeTitle || '';
    }

    // Get hookId and cta based on mode
    let hookId = '1';
    let cta = '';
    
    if (mode === 'ai') {
        hookId = window.ai_freeHookId || window.freeHookId || '1';
        cta = window.ai_freeCta || window.freeCta || 'Follow for more daily tips';
    } else {
        hookId = window.user_freeHookId || window.freeHookId || '1';
        cta = window.user_freeCta || window.freeCta || '';
    }

    // Debug log
    console.log('Mode:', mode, 'Title:', title, 'HookId:', hookId, 'CTA:', cta);
    
    if (!title) {
        alert('Title is missing! Please go back to Step 1 and enter a title.');
        return;
    }
    
    let editedContent = contentArea.value;

    // For user mode, don't prepend any hook
    if (mode === 'user') {
        console.log('User mode: using content as-is without hook');
    }
    ...
    if (freeProcessBtn) freeProcessBtn.disabled = true;
    if (generateBtn) generateBtn.disabled = true;
    if (dbCreateBtn) dbCreateBtn.disabled = true;
    
    btn.innerHTML = '<span class="processing-spinner"></span> Creating Scenes...';
    status.style.display = 'block';
    status.style.background = '#fef3c7';
    status.innerHTML = '<span class="processing-spinner"></span> Creating scenes from your content...';
    ...
    // Auto-scroll to progress container
    progressContainer.scrollIntoView({ behavior: 'smooth', block: 'center' });
    ...
    try {
        ...
        if (freeProcessBtn) freeProcessBtn.disabled = false;
        if (generateBtn) generateBtn.disabled = false;
        if (dbCreateBtn) dbCreateBtn.disabled = false;
        btn.innerHTML = "🎬 Step 2: Create Scenes";
    } finally {
        clearTimeout(timeoutId);
    }
}
async function createScenesFromContent() {
    ...
    const generateBtn = document.getElementById('generateContentBtn');
    const freeProcessBtn = document.getElementById('freeProcessBtn');
    const freeCreateBtn = document.getElementById('freeCreateScenesBtn');
    const contentArea = document.getElementById('generated_content');
    const sm_select = document.getElementById('sm_id_select');
    const status = document.getElementById('status_msg');
    const progressContainer = document.getElementById('progress_container');
    ...
    if (generateBtn) generateBtn.disabled = true;
    if (freeProcessBtn) freeProcessBtn.disabled = true;
    if (freeCreateBtn) freeCreateBtn.disabled = true;
    ...
    try {
        ...
        if (generateBtn) generateBtn.disabled = false;
        if (freeProcessBtn) freeProcessBtn.disabled = false;
        if (freeCreateBtn) freeCreateBtn.disabled = false;
        btn.innerText = "🎬 Step 2: Create Scenes from Content";
    }
}
// Show custom audience input
...
async function processFreeContent() {
    ...
    const contentArea = document.getElementById('free_processed_content');
    const contentContainer = document.getElementById('free_processed_container'); // ADD THIS
    const status = document.getElementById('status_msg');
    
    const mode = document.getElementById('btn_ai_mode').classList.contains('active') ? 'ai' : 'user';
    
    let title, reelType, language, langName, storyText, hookText, ctaText, hookId, audience, goal;
    
    language = document.getElementById('free_lang_select').value;
    langName = languageScripts[language] || language;
    
    if (mode === 'ai') {
        title = document.getElementById('free_title').value.trim();
        reelType = document.getElementById('free_reel_type').value;
        const topic = document.getElementById('free_topic').value.trim();
        audience = document.getElementById('free_audience').value;
        const audienceCustom = document.getElementById('free_audience_custom').value;
        goal = document.getElementById('free_goal_select').value;
        const ctaInput = document.getElementById('free_cta_text').value.trim();
        
        if (!topic) { alert("Please enter a topic for your video"); return; }
        if (!title) { alert("Please enter a title"); return; }
        
        // ===== FIX 1: Hook selection - filter out empty option, use .text not .value =====
        const hookSelect = document.getElementById('free_hook_select');
        const rndHook = document.getElementById('free_rnd_hook').checked;
        const validOptions = Array.from(hookSelect.options).filter(opt => opt.value !== '');
        
        if (rndHook) {
            if (validOptions.length > 0) {
                const randomHook = validOptions[Math.floor(Math.random() * validOptions.length)];
                hookText = randomHook.text;  // hook NAME shown in dropdown
                hookId = randomHook.value;   // hook NAME as value
            } else {
                hookText = 'Identity Hook';
                hookId = 'Identity Hook';
            }
        } else {
            if (hookSelect.selectedIndex > 0) {
                hookText = hookSelect.options[hookSelect.selectedIndex].text;
                hookId = hookSelect.value;
            } else {
                hookText = 'Identity Hook';
                hookId = 'Identity Hook';
        ...
        const audienceText = audience === 'custom' ? audienceCustom : 
            hookSelect.options[hookSelect.selectedIndex]?.text || audience;
        
        const audienceLabel = document.getElementById('free_audience').value === 'custom' 
            ? audienceCustom 
            : document.getElementById('free_audience').options[document.getElementById('free_audience').selectedIndex].text;

        const goalText = document.getElementById('free_goal_select').options[document.getElementById('free_goal_select').selectedIndex].text;

        ctaText = ctaInput || 'Follow for more daily tips';
        
        // Store values
        window.ai_freeTitle = title;
        window.ai_freeReelType = reelType;
        window.ai_freeHookId = hookId;
        window.ai_freeCta = ctaText;
        window.ai_freeHookText = hookText;
        window.freeTitle = title;
        window.freeReelType = reelType;
        window.freeHookId = hookId;
        window.freeCta = ctaText;
        window.freeHookText = hookText;
        
        storyText = `Topic: ${topic}\nAudience: ${audienceLabel}\nGoal: ${goalText}`;
        window.freeStoryText = storyText;
        window.ai_freeStoryText = storyText;

        // ===== FIX 2: Show debug info before sending =====
        const logDiv = document.getElementById('realtime_log');
        if (logDiv) {
            logDiv.innerHTML = `<div class="log-entry info">
                🔍 DEBUG: Hook = "${hookText}" | Lang = "${langName}" (${language}) | Title = "${title}"
            </div>`;
        }
        ...
        status.style.display = 'block';
        status.style.background = '#fef3c7';
        status.innerHTML = `<span class="processing-spinner"></span> Processing... Hook: <strong>${hookText}</strong> | Lang: <strong>${langName}</strong>`;

        if (reelType === 'single-scene') {
            await new Promise(resolve => setTimeout(resolve, 500));
            contentArea.value = `${hookText}\n\n${topic}\n\n${ctaText}`;
            
            // SHOW the container with label and textarea
        ...
        // ===== FIX 3: Language instruction added explicitly at top of prompt =====
        const processingPrompt = `You are a world-class content strategist and copywriter. Create an extremely engaging social media reel script of 30 seconds

LANGUAGE: Write the ENTIRE script in ${langName} only. Every single word must be in ${langName}. Do NOT use English unless the language IS English.

Use the opening hook style: "${hookText}". Adapt it creatively to fit the title and topic. Do not copy word for word. Open with the same psychological pattern but make it relevant to the title and topic.

Focus on the title: "${title}" and the topic: "${document.getElementById('free_topic').value.trim()}"
Tailor the script fully for the target audience: "${audienceLabel}", and work toward achieving the goal: "${goalText}"
Naturally include the key takeaway or offer: "${ctaText}" as the closing call-to-action.

**Instructions for the script**
- script must be for video of 30 seconds
1. Open with the hook style adapted to the title/topic.
2. Address the topic and title fully.
3. Highlight main pain points the audience experiences.
4. Show relatable symptoms or situations the audience recognizes.
5. Suggest possible solutions that naturally lead to the key takeaway.
6. Use short, punchy sentences (max 10 words) for social media impact.
7. Break sentences at natural pauses: commas, 'and', 'or'.
8. Add SSML pauses: <break time="0.2s"/> <break time="0.4s"/> <break time="0.6s"/> <break time="0.8s"/>
9. End with the key takeaway / offer as a strong CTA.
10. Do NOT include headings, labels, or meta text like "HOOK:", "TITLE:", etc.

**LANGUAGE RULES - CRITICAL:**
Write the ENTIRE script in ${langName} only.
Every single word must be in ${langName}.
Do NOT use any other language.
Do NOT default to Urdu or any other language.

OUTPUT ONLY the processed text with SSML pause tags. After EVERY pause tag add a newline character so each sentence appears on its own line. No headings, labels, or explanations.`;

        // Show full prompt in log for debugging
        if (logDiv) {
            logDiv.innerHTML += `<div class="log-entry info" style="white-space:pre-wrap; word-break:break-word; max-height:200px; overflow:auto;">
=== FULL PROMPT SENT TO AI ===\n${processingPrompt.replace(/</g, '&lt;')}
            </div>`;
        }
        ...
        try {
            ...
            if (data.success) {
                ...
                document.getElementById('freeCreateScenesBtn').style.display = 'block';
                status.style.background = '#dcfce7';
                status.innerHTML = "✅ Content processed! You can edit above, then click Step 2.";
            } else {
            ...
            console.error('Error:', err);
            status.style.background = '#fee2e2';
            status.innerHTML = err.name === 'AbortError' 
        ...
        // ===== USER MODE =====
        title = document.getElementById('free_user_title').value.trim();
        reelType = document.getElementById('free_user_reel_type').value;
        storyText = document.getElementById('free_story_text_user').value.trim();
        
        if (!title) { alert("Please enter a title"); return; }
        if (!storyText) { alert("Please paste your content"); return; }
        ...
        window.user_freeTitle = title;
        window.user_freeReelType = reelType;
        window.user_freeStoryText = storyText;
        window.user_freeHookId = hookId;
        window.user_freeCta = ctaText;
        window.freeTitle = title;
        window.freeReelType = reelType;
        window.freeStoryText = storyText;
        window.freeHookId = hookId;
        ...
        status.style.display = 'block';
        status.style.background = '#fef3c7';
        status.innerHTML = '<span class="processing-spinner"></span> Processing your content...';
        ...
        const processingPrompt = `You are an expert content formatter. 
		Your ONLY task is to take the text found between the <CONTENT> tags below and add SSML pause tags.
		...
		**CRITICAL RULES:**
		1. OUTPUT ONLY the processed text from inside the <CONTENT> tags.
		2. DO NOT include "TITLE:", "USER'S CONTENT:", "SCENE:", or any labels in your response.
		3. DO NOT change ANY of the user's words or meaning.
		4. ONLY add Azure SSML pause tags: <break time="0.2s"/>, <break time="0.4s"/>, <break time="0.6s"/>, <break time="0.8s"/>.
		5. Break into short sentences (max 10 words per line).
		6. Insert a line break (\n) after EVERY pause tag.

		NO EXPLANATIONS. NO INTROS. START IMMEDIATELY WITH THE FORMATTED CONTENT.`;
