    ...
    if ($action === 'get_files') {
        ...
        $res   = mysqli_query($conn,
            "SELECT image_name, id, status, media_type_format, thumbnail,
             ...
             media_type, admin_id, skip_embedding
...
<div class="page-header">
    ...
    <div id="dbStatsBar" style="margin-top:12px;padding:8px 12px;background:#f0f6ff;border:1px solid var(--bdr);border-radius:8px;font-size:12px;color:var(--mut);">
        📊 Loading DB totals…
    </div>
</div>
...
function updateStats() {
    fetch('?action=get_db_stats').then(function(r){ return r.json(); }).then(function(d){
        ...
        var el = document.getElementById('dbStatsBar');
        if (el) {
...
function renderGrid() {
    ...
    grid.innerHTML = allFiles.map(function(f){
        ...
        var thumb = f.kind === 'video'
            ? (f.thumbnail ? '<img src="podcast_thumbnails/' + encodeURIComponent(f.thumbnail) + '">' : '<video src="' + src + '" muted preload="metadata"></video><div class="vid-badge">▶️</div>')
            : (f.thumbnail ? '<img src="podcast_thumbnails/' + encodeURIComponent(f.thumbnail) + '">' : '<img src="' + src + '" loading="lazy">');
