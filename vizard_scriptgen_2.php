...
// Mirrors fal_webhook.php's fw_kick_cron(). cron_video_gen.php, when run
// from the CLI with no args, runs falFinishReadyJobs() then
// falSubmitPendingJobs() which POSTs every pending (flag=1) fal.ai job to
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'start_ai_videos') {
    ...
    while ($scenes_q && ($s = mysqli_fetch_assoc($scenes_q))) {
        ...
        $ifo_e = mysqli_real_escape_string($conn, $s['image_folder'] ?? '');
        $ai = mysqli_fetch_assoc(mysqli_query($conn,
            "SELECT is_ai_generated FROM hdb_image_data WHERE image_name='$ifn_e' AND image_folder='$ifo_e' LIMIT 1"));
        if (empty($ai['is_ai_generated'])) { $skipped++; continue; }
...
async function generateAllScenes() {
    ...
    // podcastId / storyIdMap are module-level (declared near _storyIdMap below)
    // so generateAllVideos() and the video poller can read them after this runs.
    podcastId  = 0;
    storyIdMap = {};  // { scene_num: story_id }
