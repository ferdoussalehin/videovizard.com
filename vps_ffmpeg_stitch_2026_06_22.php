...
// ── Gap to leave after a scene's audio finishes playing (seconds) ──
// Applied to both image scenes (Ken Burns) and video scenes that have
// narration audio, so there's a brief pause before the next scene cuts in.
define('SCENE_GAP_SEC', 0.25);
...
function image_to_video(
    ...
    // $duration here is the "speaking" length (audio duration, or the 5s
    // default for silent scenes). If there's audio, tack on SCENE_GAP_SEC
    // of extra hang-time at the end (still panning/zooming) so the scene
    // doesn't cut away the instant the narration stops.
    $has_audio = ($audio_path && file_exists($audio_path));
    $gap       = $has_audio ? SCENE_GAP_SEC : 0.0;
    $duration  = $duration + $gap; // total on-screen time for this image
    ...
    //
    // Pattern: scale up to 8000px wide so zoompan has room to crop,
    // then zoompan to target WxH at $fps fps for $frames frames.
    //   z   = zoom level (1.0 = no zoom, 1.12 = 12% zoom in)
    //   on  = current output frame number (0-based)
    //   d   = total frame count
    //   iw/ih = input width/height after scale=8000
    //
    // All expressions are quote-free so escapeshellarg() wraps them safely.

    $f = $frames; // short alias for expressions
    // zoompan expressions are quote-free so they survive escapeshellarg()
    // FFmpeg accepts unquoted filter expressions — no single quotes needed
    // NOTE: commas inside these expressions must be escaped with a backslash.
    // FFmpeg's filtergraph parser (not the shell) treats ',' as the filter-chain
    // separator, so an unescaped comma inside min(...) makes it think the
    // zoompan filter ends there, breaking the rest of the -vf string
    // (this was the cause of "No option name near 'x=...'" on image scenes).
    $z_in_slow  = "min(zoom+0.0006\\,1.10)";
    $z_in_fast  = "min(zoom+0.0010\\,1.12)";
    $z_out      = "1.12-0.0010*on";
    $x_left     = "0";
    $x_right    = "iw-iw/zoom";
    $x_centre   = "iw/2-(iw/zoom/2)";
    $y_top      = "0";
    $y_bottom   = "ih-ih/zoom";
    $y_centre   = "ih/2-(ih/zoom/2)";
    $zp_base    = "s={$W}x{$H}:fps={$fps}:d={$f}";

    $variants = [
        // 0: zoom in, pan top-left corner
        "scale=8000:-1,zoompan=z={$z_in_fast}:x={$x_left}:y={$y_top}:{$zp_base}",
        // 1: zoom in, pan top-right corner
        "scale=8000:-1,zoompan=z={$z_in_fast}:x={$x_right}:y={$y_top}:{$zp_base}",
        // 2: zoom in, pan bottom-left corner
        "scale=8000:-1,zoompan=z={$z_in_fast}:x={$x_left}:y={$y_bottom}:{$zp_base}",
        // 3: zoom in, pan bottom-right corner
        "scale=8000:-1,zoompan=z={$z_in_fast}:x={$x_right}:y={$y_bottom}:{$zp_base}",
        // 4: slow zoom in, stay centred
        "scale=8000:-1,zoompan=z={$z_in_slow}:x={$x_centre}:y={$y_centre}:{$zp_base}",
        // 5: zoom out, stay centred
        "scale=8000:-1,zoompan=z={$z_out}:x={$x_centre}:y={$y_centre}:{$zp_base}",
    ];
...
if (isset($_POST['action']) && $_POST['action'] === 'start_stitch') {
    ...
    while ($s = mysqli_fetch_assoc($sq)) {
        ...
        //   2. Strip _thumb suffix → look in user_media/ and podcast_images/
        //   3. Download original from web server
        ...
        if ($is_image && !file_exists($vabs)) {
            ...
            if ($orig_file !== $vfile) {
                // Look for original in user_media folders
    ...
    foreach ($scenes as $i => $scene) {
        ...
        if ($scene['is_image']) {
            // Duration = audio length if available, else 5s default
            $duration = $scene['aabs']
                ? get_audio_duration($FFMPEG, $scene['aabs'])
                : 5.0;
        ...
        // ── VIDEO SCENE: trim to audio length + burn captions ─
            ...
            if ($scene['aabs']) {
                // Freeze the last video frame for SCENE_GAP_SEC so the clip
                // doesn't get cut off the instant the narration ends, and pad
                // the audio with the same amount of silence to match.
                $gap_af  = number_format(SCENE_GAP_SEC, 3, '.', '');
                $vf_gap  = $vf . ',tpad=stop_mode=clone:stop_duration=' . $gap_af;
                $total_t = number_format(5 + SCENE_GAP_SEC, 3, '.', '');
                $ffmpeg_logs[] = "Scene {$scene['seq']} [VIDEO] VF: " . $vf_gap;

                $clip_cmd = escapeshellcmd($FFMPEG)
                    . ' -y'
                    . ' -ss 0 -t 5 -i ' . escapeshellarg($scene['vabs'])
                    . ' -ss 0 -t 5 -i ' . escapeshellarg($scene['aabs'])
                    . ' -vf ' . escapeshellarg($vf_gap)
                    . ' -af ' . escapeshellarg("apad=pad_dur={$gap_af}")
                    . ' -t ' . $total_t
                ...
                $ffmpeg_logs[] = "Scene {$scene['seq']} [VIDEO] VF: " . $vf;
                $clip_cmd = escapeshellcmd($FFMPEG)
                    . ' -y'
                    . ' -ss 0 -t 5 -i ' . escapeshellarg($scene['vabs'])
                    . ' -vf ' . escapeshellarg($vf)
...
function renderScenes(scenes, title) {
  ...
  scenes.forEach((s, i) => {
    const folder = (s.image_folder || 'podcast_images').replace(/\/$/, '');
