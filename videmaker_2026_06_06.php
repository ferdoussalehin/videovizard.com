...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'save_rendered_video') {
    ...
    try {
        ...
        // Handle binary file upload (preferred — no base64 overhead)
        ...
        // Fallback: base64 encoded in POST body (legacy)

        } elseif (!empty($_POST['video_data'])) {

            $video_data = preg_replace('/^data:video\/\w+;base64,/', '', $_POST['video_data']);

            $video_data = base64_decode($video_data);

            if ($video_data === false) throw new Exception('Failed to decode video data');
...
function finishRecording() {
    ...
    // Snapshot chunks NOW before anything clears them

    const chunksSnapshot = recordedChunks.slice();

    const mimeSnapshot = (mediaRecorder && mediaRecorder.mimeType) ? mediaRecorder.mimeType : 'video/webm';

    console.log(`🎥 finishRecording: chunks=${chunksSnapshot.length}, mediaRecorder state=${mediaRecorder ? mediaRecorder.state : 'null'}`);



    if (mediaRecorder && mediaRecorder.state !== 'inactive') {

        // onstop will fire — pass chunks via closure so nothing gets cleared first

        mediaRecorder.onstop = () => {

            console.log('⏹️ MediaRecorder onstop fired');

            saveRecordedVideoFromChunks(chunksSnapshot, mimeSnapshot);
        ...
        // Already inactive — save directly

        console.log('🎥 MediaRecorder already inactive — saving directly');
    ...
    // Restore overlays and reset to scene 1
...
// Save the recorded video — accepts explicit chunks and mimeType

// so it works regardless of global mediaRecorder/recordedChunks state

function saveRecordedVideoFromChunks(chunks, mimeType) {

    console.log('💾 saveRecordedVideoFromChunks called');

    console.log('💾 chunks:', chunks.length, '| mimeType:', mimeType);

    console.log('💾 total size:', chunks.reduce((a, c) => a + c.size, 0), 'bytes');
    ...
    if (blob.size === 0) {
        ...
        alert('Video blob is empty — nothing was captured. Please try again.');
    ...
    // Use .webm extension — browsers record as webm regardless of filename

    const podcastId = <?= $url_podcast_id ?: 0 ?>;

    const langCode = '<?= $podcast_lang_code ?>';

    const filename = `video_${podcastId}_${langCode}.webm`;
    ...
    // ── 2. Upload binary blob to server ──────────────────────

    L('⬆️ Uploading to server...');

    console.log('💾 Uploading to server...');

    const formData = new FormData();

    formData.append('ajax_action', 'save_rendered_video');
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            L(`✅ Saved to server: ${data.filepath}`);

            alert(`✅ Video rendered successfully!\n\nFilename: ${filename}\nSize: ${(blob.size / (1024*1024)).toFixed(2)} MB\nScenes: ${scenes.length}\nSaved to: ${data.filepath}`);

        } else {

            L(`⚠️ Server save failed: ${data.message}`);

            alert(`✅ Video downloaded!\n\n⚠️ Server save failed: ${data.message}\n\nYou still have the downloaded copy.`);
    ...
    .catch(err => {
        ...
        L(`⚠️ Upload error: ${err.message}`);

        alert(`✅ Video downloaded!\n\n⚠️ Could not save to server: ${err.message}`);
...
// Legacy wrapper — keeps onstop in startMediaRecorder working
...
function saveRecordedVideo() {
    ...
    saveRecordedVideoFromChunks(chunks, mime);
