...
<div id="my-idea-tab" class="tab-content">
    ...
    <div id="idea_subtab_ai">
        <div class="form-group">
            <label>📌 Topic</label>
            <input type="text" id="idea_topic_ai" placeholder="e.g., Stress Relief Techniques, Morning Motivation..." value="Stress Relief">
        </div>
...
<!-- Log controls - Only show for super admin -->
<?php if ($_SESSION['level'] === 'super'): ?>
...
<!-- Debug Panel - Only show for super admin -->
<?php if ($_SESSION['level'] === 'super'): ?>
...
<!-- Step 2 Setup Panel -->
<div id="step2SetupPanel" style="display:none;" class="card" style="margin-bottom:20px;">
    <div class="card-body">
        ...
        <div class="form-group dd-wrap" id="s2_media_dd_wrap">
            <div class="dd-trigger" id="s2_media_trigger" onclick="toggleDD('s2_media')">
                <span>🎞️ Video Media: <strong id="s2_media_label">Stock Images</strong></span>
                <span class="dd-arrow" id="s2_media_arrow">▾</span>
            </div>
            <div class="dd-panel" id="s2_media_panel">
                <button type="button" class="reel-dd-btn active" onclick="pickDD('s2_mediaVal','stock_images','Stock Images',this)">Stock Images</button>
                ...
                <button type="button" class="reel-dd-btn" onclick="pickDD('s2_mediaVal','mix_media','Mix Media',this)">Mix Media</button>
            </div>
            <input type="hidden" id="s2_mediaVal" value="stock_images">
        </div>
...
<div class="modal-overlay" id="progressModal" style="display: none;">
    <div class="success-card" style="max-width: 550px; text-align: left;">
        <h3 style="color: var(--dark-blue); margin-bottom: 20px; display: flex; align-items: center; gap: 10px;">
            <span>🎬</span> Creating Your Video
        </h3>
        
        <!-- Progress Steps -->
        <div style="margin-bottom: 25px;">
            <div style="display: flex; align-items: center; margin-bottom: 15px;">
                <div id="step1Icon" style="width: 30px; height: 30px; border-radius: 50%; background: #e2e8f0; display: flex; align-items: center; justify-content: center; margin-right: 12px; font-size: 14px; color: white;">1</div>
                <div style="flex: 1;">
                    <div style="font-weight: 600; color: var(--dark-blue);" id="step1Text">Creating scenes in database</div>
                    <div style="font-size: 12px; color: var(--muted);" id="step1Detail">Initializing...</div>
                </div>
                <div id="step1Status" style="font-size: 20px;">⏳</div>
            </div>
            
            <div style="display: flex; align-items: center; margin-bottom: 15px;">
                <div id="step2Icon" style="width: 30px; height: 30px; border-radius: 50%; background: #e2e8f0; display: flex; align-items: center; justify-content: center; margin-right: 12px; font-size: 14px; color: white;">2</div>
                <div style="flex: 1;">
                    <div style="font-weight: 600; color: var(--dark-blue);" id="step2Text">Generating audio</div>
                    <div style="font-size: 12px; color: var(--muted);" id="step2Detail">Waiting to start...</div>
                </div>
                <div id="step2Status" style="font-size: 20px;">⏳</div>
            </div>
            
            <div style="display: flex; align-items: center; margin-bottom: 15px;">
                <div id="step3Icon" style="width: 30px; height: 30px; border-radius: 50%; background: #e2e8f0; display: flex; align-items: center; justify-content: center; margin-right: 12px; font-size: 14px; color: white;">3</div>
                <div style="flex: 1;">
                    <div style="font-weight: 600; color: var(--dark-blue);" id="step3Text">Assigning images/videos</div>
                    <div style="font-size: 12px; color: var(--muted);" id="step3Detail">Waiting to start...</div>
                </div>
                <div id="step3Status" style="font-size: 20px;">⏳</div>
            </div>
        </div>
        
        <!-- Overall Progress Bar -->
        <div style="background: #f1f5f9; border-radius: 20px; height: 8px; width: 100%; margin: 20px 0; overflow: hidden;">
            <div id="overallProgressBar" style="background: linear-gradient(90deg, #8b5cf6, #6366f1); height: 100%; width: 0%; transition: width 0.3s ease;"></div>
        </div>
        
        <!-- Current Action -->
        <div style="background: #f8fafc; padding: 15px; border-radius: 10px; margin: 20px 0;">
            <div style="font-weight: 600; color: var(--dark-blue); margin-bottom: 5px;" id="currentAction">Initializing...</div>
            <div style="font-size: 13px; color: var(--muted);" id="currentActionDetail">Please wait</div>
        </div>
        
        <!-- Scene Progress -->
        <div style="margin-top: 10px; font-size: 13px; color: var(--muted); text-align: center;" id="sceneProgress">
            Processing scene 0/0
        </div>
        
        <!-- Cancel Button -->
        <div style="margin-top: 25px;">
            <button class="btn btn-secondary" onclick="cancelSceneCreation()" style="width: 100%;" id="cancelSceneBtn">Cancel Process</button>
        </div>
    </div>
</div>
<script>
...
- Aim for ${Math.ceil(targetWords/10)}-${Math.ceil(targetWords/5)} scenes`;
    ...
    // FIX: Changed from const to let since we're building it conditionally
...
function switchTab(tab) {
    ...
    if (tab === 'my-idea') {
        ...
        checkPodcastType('db');
        setTimeout(addTrashIconsToDropdowns, 500);
        // Auto-load first niche's topics when switching to database tab
...
async function processIdeaContent(retryCount = 0) {
    ...
    let topic = '';
    if (aiSubtab && aiSubtab.style.display !== 'none') {
        // AI sub-tab: use simple topic input
        topic = topicAI ? topicAI.value.trim() : '';
    ...
    if (!audienceSelect || !ctaInput || !langSelect) {
...
async function assignImagesToAllScenes(podcastId, mediaType = 'stock_images') {
    L(`🖼️ ===== STARTING MEDIA ASSIGNMENT (type: ${mediaType}) =====`, 'info');
    // Check if cancelled
		if (window.stopSceneCreation) {
    ...
    for (let i = 0; i < scenes.length; i++) {
        ...
        // ── Determine what to fetch based on mediaType ──
        // stock_images  → images only from library
        // stock_videos  → videos only from library  
        // unique_images → skip library, always AI generate
        // mix_media     → images + videos from library (original behaviour)

        let assignedImage = null;

        if (mediaType === 'unique_images') {
            // Always generate AI image, skip library search
            L(`🎨 AI Images mode — generating for scene ${sceneNumber}...`, 'info');
            ...
            if (generated && generated.success) {
                ...
                L(`✅ AI image generated & assigned: ${generated.filename}`, 'success');
            } else {
                L(`❌ AI generation failed: ${generated?.message || 'Unknown error'}`, 'error');
            }
        } else {
            // Search library
            ...
            if (results && results.length > 0) {
                L(`📊 Found ${results.length} total matches`, 'info');

                const availableResults = results.filter(r => !usedMedia.has(r.filename));
                duplicatesRejected += results.length - availableResults.length;

                // Filter by mediaType
                let candidates = availableResults;
                if (mediaType === 'stock_images') {
                    candidates = availableResults.filter(r => r.type === 'image');
                    L(`🖼️ Stock Images mode — ${candidates.length} images available`, 'info');
                } else if (mediaType === 'stock_videos') {
                    candidates = availableResults.filter(r => r.type === 'video');
                    L(`🎬 Stock Videos mode — ${candidates.length} videos available`, 'info');
                } else {
                    // mix_media — prefer videos 70/30
                    L(`🎞️ Mix Media mode — ${availableResults.filter(r=>r.type==='video').length} videos, ${availableResults.filter(r=>r.type==='image').length} images`, 'info');
                }

                if (candidates.length === 0) {
                    // None of selected type available — fall back to AI generation
                    L(`⚠️ No ${mediaType} available — falling back to AI generation`, 'warning');
                    const generated = await generateImageForScene(scene, sceneNumber, total);
                    if (generated && generated.success) {
                        generatedAssigned++;
                        assignedImage = generated.filename;
                        usedMedia.add(generated.filename);
                        await assignImageToScene(scene.id, generated.filename);
                        L(`✅ AI fallback assigned: ${generated.filename}`, 'success');
                    }
                } else {
                    ...
                    if (mediaType === 'mix_media') {
                        ...
                        const images = candidates.filter(r => r.type === 'image');
                        if (videos.length > 0 && (Math.random() < 0.7 || images.length === 0)) {
                            selectedItem = videos[Math.floor(Math.random() * videos.length)];
                            videosAssigned++;
                        } else if (images.length > 0) {
                            selectedItem = images[Math.floor(Math.random() * images.length)];
                            imagesAssigned++;
                        }
                    } else {
                    ...
                    if (selectedItem) {
                        ...
                        usedMedia.add(assignedImage);
                        const assigned = await assignImageToScene(scene.id, assignedImage);
                        if (assigned) {
                            L(`✅ Assigned ${selectedItem.type}: ${assignedImage}`, 'success');
                        } else {
                ...
                // Nothing in library
                L(`⚠️ No matches in library — generating AI image`, 'warning');
                document.getElementById('progress_text').innerHTML = `Scene ${sceneNumber}/${total}: Generating AI image...`;
                const generated = await generateImageForScene(scene, sceneNumber, total);
                if (generated && generated.success) {
                    generatedAssigned++;
                    ...
                    usedMedia.add(generated.filename);
                    await assignImageToScene(scene.id, generated.filename);
                    L(`✅ AI image assigned: ${generated.filename}`, 'success');
                } else {
                    L(`❌ AI generation failed`, 'error');
                }
            }
        }
...
function showProgressModal() {
    ...
    if (modal) {
        // Reset all steps
        updateStepStatus(1, 'pending', 'Creating scenes in database', 'Waiting to start...');
        updateStepStatus(2, 'pending', 'Generating audio', 'Waiting to start...');
        updateStepStatus(3, 'pending', 'Assigning images/videos', 'Waiting to start...');
        
        document.getElementById('overallProgressBar').style.width = '0%';
        document.getElementById('currentAction').textContent = 'Initializing...';
        document.getElementById('currentActionDetail').textContent = 'Please wait';
        document.getElementById('sceneProgress').textContent = 'Processing scene 0/0';
...
function updateOverallProgress(percent, action, detail, sceneProgress) {
    ...
    document.getElementById('currentActionDetail').textContent = detail;
    if (sceneProgress) {
        document.getElementById('sceneProgress').textContent = sceneProgress;
    }
}
...
async function generateTopicIdeas() {
    ...
    const prompt = `You are an expert content strategist specializing in niche audience targeting.

Generate 20 specific, detailed MICRO-NICHES within the broader category of "${nicheName}".

IMPORTANT: These should be SUB-NICHES / MICRO-NICHES, not video titles. Each micro-niche should target a very specific audience segment with unique needs.

For example, if the main niche is "Parenting", generate micro-niches like:
- Newborn Sleep Training (Surviving the first 3 months)
- Toddler Tantrum Taming (Gentle discipline techniques)
- Screen Time Boundaries (Managing digital devices for kids)
- Picky Eater Solutions (Nutrition for fussy toddlers)
- Single Parenting Support (Thriving as a solo parent)
- Work-From-Home with Kids (Balancing career and childcare)
- Teen Mental Health (Navigating adolescence and anxiety)
- Special Needs Parenting (Resources for unique challenges)
- Positive Discipline (Alternatives to punishment)
- Mom Guilt & Self-Care (Prioritizing parent mental health)

Each micro-niche MUST:
1. Target a SPECIFIC sub-audience within ${nicheName}
2. Address a UNIQUE pain point, goal, or interest
3. Be detailed enough that someone could create 50+ videos on that specific topic
4. Include a brief description in parentheses explaining the focus

Return ONLY a valid JSON array of strings, each being a complete micro-niche description.
Format example: ["Micro-niche 1 (description)", "Micro-niche 2 (description)", ...]

Generate 20 diverse, specific micro-niches for "${nicheName}" now:`;
...
function displayTopicList(topicsWithStatus, nicheName) {
    ...
    topicsWithStatus.forEach((item, index) => {
        ...
        itemDiv.style.cssText = 'margin-bottom: 10px; border: 1px solid var(--border); border-radius: 8px; overflow: hidden; background: white; width: 100%;';
        ...
        // Create container with full width and proper flex layout
        const containerDiv = document.createElement('div');
        containerDiv.style.cssText = 'display: flex; align-items: center; padding: 10px; gap: 10px; width: 100%; box-sizing: border-box;';
        ...
        if (item.isDuplicate) checkbox.disabled = true;
        checkbox.style.cssText = 'width: 20px; height: 20px; flex-shrink: 0; accent-color: var(--purple); cursor: pointer;';
        checkbox.onchange = updateSelectedTopicsCount;
        
        // Input field - with flex-grow to take remaining space
        const input = document.createElement('input');
        input.type = 'text';
        input.className = 'topic-edit-input';
        input.setAttribute('data-index', index);
        input.value = displayText;
        if (item.isDuplicate) input.disabled = true;
        input.style.cssText = 'flex: 1; min-width: 0; padding: 8px 12px; border: 2px solid var(--border); border-radius: 6px; font-size: 14px; background: ' + (item.isDuplicate ? '#f1f5f9' : 'white') + '; box-sizing: border-box;';
        input.onfocus = function() { this.style.borderColor = 'var(--purple)'; };
        input.onblur = function() { this.style.borderColor = 'var(--border)'; };
        
        // Duplicate warning span
        let warningSpan = null;
        if (item.isDuplicate) {
            warningSpan = document.createElement('span');
            warningSpan.style.cssText = 'color: #ef4444; font-size: 12px; flex-shrink: 0; margin-left: 5px;';
            warningSpan.textContent = '(already exists)';
        }
        ...
        deleteBtn.innerHTML = '🗑️';
        deleteBtn.style.cssText = 'background: none; border: none; color: #ef4444; cursor: pointer; font-size: 18px; padding: 4px 8px; border-radius: 4px; opacity: 0.7; transition: opacity 0.2s; flex-shrink: 0;';
        deleteBtn.onmouseover = function() { this.style.opacity = '1'; };
        deleteBtn.onmouseout = function() { this.style.opacity = '0.7'; };
        ...
        if (warningSpan) containerDiv.appendChild(warningSpan);
...
function displayTitleList(titles, topicName) {
    console.log('displayTitleList called with', titles.length, 'titles');
    
    const listDiv = document.getElementById('titleList');
    if (!listDiv) {
        console.error('Title list element not found');
        return;
    }
    ...
    if (!titles || !Array.isArray(titles) || titles.length === 0) {
        listDiv.innerHTML = '<div style="padding: 30px; text-align: center; color: var(--muted);">No titles to display</div>';
        return;
    }
    ...
    titles.forEach((item, index) => {
        ...
        itemDiv.style.cssText = 'margin-bottom: 15px; border: 1px solid var(--border); border-radius: 12px; overflow: hidden; background: white;';
        
        // Title header with checkbox and editable title
        const headerDiv = document.createElement('div');
        headerDiv.style.cssText = 'padding: 15px; background: #f8fafc; border-bottom: 1px solid var(--border); display: flex; align-items: center; gap: 12px;';
        
        headerDiv.innerHTML = `
            <input type="checkbox" class="title-checkbox" data-index="${index}" 
                   style="width: 20px; height: 20px; accent-color: var(--purple); cursor: pointer;" 
                   onchange="updateSelectedTitlesCount()">
            <input type="text" class="title-edit-input" data-index="${index}" 
                   value="${escapeHtml(item.title)}" 
                   style="flex: 1; padding: 8px 12px; border: 2px solid var(--border); border-radius: 8px; font-size: 14px; background: white; transition: border-color 0.2s;"
                   onfocus="this.style.borderColor='var(--purple)'"
                   onblur="this.style.borderColor='var(--border)'"
                   placeholder="Edit title...">
            <button class="delete-title-btn" onclick="deleteTitleFromList(this, ${index})" 
                    style="background: none; border: none; color: #ef4444; cursor: pointer; font-size: 20px; padding: 4px 8px; border-radius: 4px; opacity: 0.7; transition: opacity 0.2s;"
                    ...
                    title="Delete this title">🗑️</button>
        `;
        
        // CTAs container with editable fields
        const ctasDiv = document.createElement('div');
        ctasDiv.style.cssText = 'padding: 15px; background: white; display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px;';
        ...
        ctasDiv.innerHTML = `
            <div style="background: #f0f9ff; padding: 12px; border-radius: 8px;">
                <div style="font-size: 11px; font-weight: 600; color: #0369a1; margin-bottom: 6px; text-transform: uppercase;">💬 ENGAGEMENT</div>
                <input type="text" class="cta-edit-input" data-title-index="${index}" data-cta-type="engagement" 
                       value="${escapeHtml(ctas.engagement)}" 
                       style="width: 100%; padding: 8px; border: 2px solid #bae6fd; border-radius: 6px; font-size: 12px; background: white;"
                       onfocus="this.style.borderColor='#0369a1'"
                       onblur="this.style.borderColor='#bae6fd'">
            </div>
            <div style="background: #f0fdf4; padding: 12px; border-radius: 8px;">
                <div style="font-size: 11px; font-weight: 600; color: #166534; margin-bottom: 6px; text-transform: uppercase;">🔗 CONVERSION</div>
                <input type="text" class="cta-edit-input" data-title-index="${index}" data-cta-type="conversion" 
                       value="${escapeHtml(ctas.conversion)}" 
                       style="width: 100%; padding: 8px; border: 2px solid #bbf7d0; border-radius: 6px; font-size: 12px; background: white;"
                       onfocus="this.style.borderColor='#166534'"
                       onblur="this.style.borderColor='#bbf7d0'">
            </div>
            <div style="background: #fef3c7; padding: 12px; border-radius: 8px;">
                <div style="font-size: 11px; font-weight: 600; color: #92400e; margin-bottom: 6px; text-transform: uppercase;">⏱️ RETENTION</div>
                <input type="text" class="cta-edit-input" data-title-index="${index}" data-cta-type="retention" 
                       value="${escapeHtml(ctas.retention)}" 
                       style="width: 100%; padding: 8px; border: 2px solid #fde68a; border-radius: 6px; font-size: 12px; background: white;"
...
async function saveSelectedTitles() {
    ...
    selectedCheckboxes.forEach(cb => {
        ...
        // Get edited title from input field
        const titleInput = document.querySelector(`.title-edit-input[data-index="${index}"]`);
        if (!titleInput || !titleInput.value.trim()) return;
        
        // Get edited CTAs from input fields
        const engagementInput = document.querySelector(`.cta-edit-input[data-title-index="${index}"][data-cta-type="engagement"]`);
        const conversionInput = document.querySelector(`.cta-edit-input[data-title-index="${index}"][data-cta-type="conversion"]`);
        const retentionInput = document.querySelector(`.cta-edit-input[data-title-index="${index}"][data-cta-type="retention"]`);
        
        // Use edited values or fallback to original
        const engagement = engagementInput ? engagementInput.value.trim() : 
                          (window.currentTitleData.titles[index]?.ctas?.engagement || 'Comment below your thoughts! 💬');
        
        const conversion = conversionInput ? conversionInput.value.trim() : 
                          (window.currentTitleData.titles[index]?.ctas?.conversion || 'Follow for more tips! ➡️');
        
        const retention = retentionInput ? retentionInput.value.trim() : 
                         (window.currentTitleData.titles[index]?.ctas?.retention || 'Watch till the end! ⏳');
        
        selectedTitles.push({
            title: titleInput.value.trim(),
            hook_type: window.currentTitleData.titles[index]?.hook_type || 'mixed',
            ctas: {
    ...
    console.log('Selected titles to save (with edited values):', selectedTitles);
...
function loadTopicsByNiche(nicheId) {
    ...
    .then(data => {
        ...
        if (data.success && data.topics && data.topics.length > 0) {
            // Build dropdown with topics for this niche
            ...
            options += '<option value="__manual__">➕ Enter custom topic...</option>';
            
            topicSelect.innerHTML = options;
            
            // Try to restore previous selection if it exists in this niche
            ...
            // No topics found for this niche
            topicSelect.innerHTML = '<option value="">-- No topics for this niche --</option><option value="__manual__">➕ Enter custom topic...</option>';
    ...
    .catch(err => {
        ...
        topicSelect.disabled = false;
        topicSelect.innerHTML = '<option value="">-- Error loading topics --</option><option value="__manual__">➕ Enter custom topic...</option>';
    });
}
...
async function generateTitleIdeas() {
    ...
    if (loadingEl) {
        ...
        const timerInterval = setInterval(() => {
            ...
            if (elapsedTimeSpan) {
                elapsedTimeSpan.textContent = `${elapsedSeconds}s elapsed`;
            }
        }, 1000);
        ...
        window.currentDotInterval = dotInterval;
    }
    ...
    if (loadingStep) loadingStep.textContent = 'Generating titles...';
    
    L(`🎯 Generating 25 title ideas for topic: ${context.topicName}`, 'info');
...
Return ONLY a valid JSON object with EXACTLY 25 items in the titles array:
{
  "titles": [
    {
      "title": "Title 1",
    ...
    try {
        ...
        if (data.success) {
            ...
            try {
                ...
                console.log('Number of titles received:', result.titles ? result.titles.length : 'no titles array');
            } catch (e) {
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    if (line && line.length > 0 && result.titles.length < 50) {
                        result.titles.push({
                            title: line,
                            hook_type: 'mixed',
                            ctas: {
            ...
            if (titlesArray.length > 0) {
                ...
                if (loadingStep) loadingStep.textContent = 'Titles generated!';
                
                // Clear intervals
                if (window.currentTimerInterval) clearInterval(window.currentTimerInterval);
                if (window.currentDotInterval) clearInterval(window.currentDotInterval);
                ...
                // Use the editable title display function
                displayTitleList(titlesArray, context.topicName);
                ...
                L(`✅ Generated ${titlesArray.length} titles`, 'success');
            } else {
        ...
        // Hide loading and clear intervals on error
        if (loadingEl) loadingEl.style.display = 'none';
        if (window.currentTimerInterval) clearInterval(window.currentTimerInterval);
        if (window.currentDotInterval) clearInterval(window.currentDotInterval);
        if (window.currentProgressInterval) clearInterval(window.currentProgressInterval);
    ...
    <div class="success-card" style="max-width: 650px;">
        ...
        <div id="topicStep2" style="display: none;">
			...
			<div style="display: flex; flex-direction: column; gap: 10px; margin-top: 20px;">
				<!-- Row 1: Cancel and Add Selected Topics -->
				<div style="display: flex; gap: 10px; width: 100%;">
					<button class="btn btn-secondary" onclick="closeTopicModal()" style="flex: 1; min-width: 0;">Cancel</button>
					<button class="btn btn-success" onclick="saveSelectedTopics()" style="flex: 1; min-width: 0;">Add Selected Topics</button>
				</div>
				
				<!-- Row 2: Generate More and Change Niche -->
				<div style="display: flex; gap: 10px; width: 100%;">
					<button class="btn btn-primary" onclick="generateMoreTopics()" style="flex: 1; min-width: 0;">Generate More Ideas</button>
					<button class="btn btn-info" onclick="changeNiche()" style="flex: 1; min-width: 0; background: #64748b; color: white;">Change Niche</button>
				</div>
			</div>
        </div>
    </div>
    ...
    <div class="success-card" style="max-width: 800px;">
        ...
        <div id="titleStep1">
			...
			<div id="titleLoading" style="display: none; text-align: center; padding: 30px 20px; background: #f1f5f9; border-radius: 12px; margin: 15px 0;">
				...
				<div style="color: #94a3b8; font-size: 12px; margin-top: 15px; display: flex; align-items: center; justify-content: center; gap: 15px;">
					<div style="display: flex; align-items: center; gap: 5px;">
						<span>⏱️</span>
						<span id="loadingTime">This may take 60+ seconds</span>
					</div>
					<div style="display: flex; align-items: center; gap: 5px;">
						<span>⏲️</span>
			...
			<div style="display: flex; gap: 10px; justify-content: flex-end; margin-top: 20px;">
				...
				<button class="btn btn-primary" id="generateTitlesBtn" onclick="generateTitleIdeas()">Generate 25 Titles</button>
			</div>
		</div>
        ...
        <div id="titleStep2" style="display: none;">
			...
			<!-- Select All Bar -->
			<div class="select-all-bar" style="background: #f1f5f9; padding: 12px 15px; border-radius: 10px; margin-bottom: 15px; display: flex; align-items: center; gap: 15px;">
				<input type="checkbox" id="selectAllTitles" onchange="toggleSelectAllTitles(this)" style="width: 20px; height: 20px; cursor: pointer; accent-color: var(--purple);">
			...
			<!-- Titles List - Simplified without hover effects on the box -->
			<div id="titleList" style="max-height: 400px; overflow-y: auto; margin-bottom: 20px; border: 1px solid var(--border); border-radius: 12px; padding: 10px; background: #fafafa;">
				<!-- Titles will be inserted here -->
			</div>
			
			<!-- Action Buttons - All on one line -->
			<div style="display: flex; gap: 10px; justify-content: flex-end; margin-top: 20px;">
				<button class="btn btn-secondary" onclick="closeTitleModal()" style="min-width: 100px;">Cancel</button>
				<button class="btn btn-success" onclick="saveSelectedTitles()" style="min-width: 160px;">Save Selected Titles</button>
				<button class="btn btn-primary" onclick="generateMoreTitles()" style="min-width: 140px;">Generate More</button>
			</div>
		</div>
    </div>
</div>
...
function showStep2Setup(source) {
    ...
    // Load voices into step 2 pickers (copy from global pickers loaded by loadVoicesForLanguage)
...
function hideStep2Setup() {
    ...
    document.querySelectorAll('.tab-content.hidden').forEach(t => t.style.display = 'none');
    const activeTab = document.querySelector('.tab-btn.active');
    if (activeTab) {
        const tabId = activeTab.getAttribute('data-tab') || activeTab.onclick?.toString().match(/showTab\('([^']+)'\)/)?.[1];
        if (tabId) document.getElementById(tabId + '-tab')?.style.setProperty('display', '');
    }
    _step2Source = null;
}
...
function loadStep2Voices(langCode) {
    ...
    if (srcHost && s2Host) {
        ...
        s2Host.value = srcHost.value;
    }
    if (srcGuest && s2Guest) {
        s2Guest.innerHTML = srcGuest.innerHTML;
        s2Guest.value = srcGuest.value;
    }
}
