...
// ── AJAX: save_company_industry — sets ai_group/ai_subgroup on hdb_companies ──
    ...
    try {
        ...
        $sql_pod = "INSERT INTO hdb_podcasts
            ...
            ($admin_id, $team_lead_id, $co_id, '$esc_title', '$lang_code', '$reel_type', 'draft', 'draft',
             ...
             1, '$pod_ai_group_e', '$pod_ai_subgroup_e')";
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'get_queue_status') {
    ...
    $img_scenes_q = mysqli_query($conn,
        "SELECT id AS scene_id, image_file AS image_name, image_folder FROM hdb_podcast_stories
...
if (isset($_POST['ajax_action']) && ($_POST['ajax_action'] === 'generate_scene_image' || $_POST['ajax_action'] === 'generate_scene_image_modal')) {
    ...
    $pod_cat_row  = mysqli_fetch_assoc(mysqli_query($conn,
        "SELECT ai_group, ai_subgroup FROM hdb_podcasts WHERE id=$podcast_id LIMIT 1"));
    ...
    // ── Save image to disk via mediaIngest() ────────────────────────
    // Routes through the same library pipeline as user uploads: lands in
    // the company-scoped folder, gets GPT-4o vision tags + an embedding,
    // and is inserted into hdb_image_data — so future scenes can reuse it
    // via findUserMediaMatch() instead of generating a near-duplicate.
    $imageData  = base64_decode($falResult['image']);
    $tmpImgPath = sys_get_temp_dir() . '/scene_img_' . getmypid() . '_' . time() . '.png';
    if (file_put_contents($tmpImgPath, $imageData) === false) {
        logGeneration("FAL HANDLER: Could not write temp image file", "FAL_ERROR");
        ob_end_clean();
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(['success' => false, 'error' => 'Could not write temp image file', 'scene' => $scene_num, 'fail_stage' => 'save']);
        exit;
    }

    $mi_context = trim($co_ai_group . ' ' . $co_ai_subgroup) ?: 'Promotional video scene';
    $ingest = mediaIngest([
        'local_path'      => $tmpImgPath,
        'admin_id'        => $admin_id,
        'company_id'      => $company_id,
        'promo_group'     => $co_ai_group,
        'promo_subgroup'  => $co_ai_subgroup,
    ...
    if (empty($ingest['success'])) {
        logGeneration("FAL HANDLER: mediaIngest FAILED | " . ($ingest['message'] ?? 'unknown'), "FAL_ERROR");
        ob_end_clean();
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(['success' => false, 'error' => 'Could not save image: ' . ($ingest['message'] ?? 'unknown'), 'scene' => $scene_num, 'fail_stage' => 'save']);
        exit;
    }
    // mediaIngest() assigns its own filename/folder — use those from here on,
    // not the deterministic scene_{podcast}_{story}.png name computed earlier.
    $image_name   = $ingest['filename'];
    $image_folder = $ingest['folder'];
    $ine = mysqli_real_escape_string($conn, $image_name);
    $ife = mysqli_real_escape_string($conn, $image_folder);
    logGeneration("FAL HANDLER: Image ingested id=" . ($ingest['image_id'] ?? '?') . " $image_folder/$image_name tagged=" . (!empty($ingest['tagged']) ? 'Y' : 'N'), "FAL");
    ...
    // Only ever queue a video job for images that were actually AI
    // generated — never for a real photo. This branch only runs right
    // after a fresh fal.ai generation, so is_ai_generated is always true
    // here, but the check stays explicit so the rule holds even if this
    // code path is ever reused for something else.
    // gen_mode is left blank — fal.ai is the only generator now, and
    // cron_video_gen.php already defaults blank/missing gen_mode to fal.ai.
    $vid_queued = false;
    $gme_blank  = '';
    if (!empty($ingest['is_ai_generated'])) {
    $vid_row = mysqli_fetch_assoc(mysqli_query($conn,
        "SELECT id, videogen_flag FROM hdb_video_gen_que WHERE podcast_id=$podcast_id AND scene_id=$story_id LIMIT 1"
    ));
    if ($vid_row) {
        ...
        logGeneration("FAL HANDLER: skipped video queue — image not AI-generated | podcast=$podcast_id scene=$story_id", "FAL");
    }
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'generate_scene_video') {
    ...
    if ($img_row && !empty($img_row['image_file'])) {
        $ifn_e = mysqli_real_escape_string($conn, $img_row['image_file']);
        $ifo_e = mysqli_real_escape_string($conn, $img_row['image_folder'] ?? '');
        ...
        $scene_is_ai_generated = !empty($img_data_row['is_ai_generated']);
    }
...
Return EXACTLY this JSON:
{
  ...
  \"emotional_outcome\": {\"selected\": \"...\", \"options\": [\"...\",\"...\",\"...\",\"...\"]}
    ...
    $additional = $_SESSION['additional_info'] ?? '';
    $promoting_item  = $_SESSION['promoting_item'] ?? '';
    $step4_input = "Business/Niche: $business" . ($promoting_item ? "\nWhat We Are Promoting Today: $promoting_item\nIMPORTANT: Anchor every field (title, hook, setting, hero_element, etc.) to THIS specific item, not the business in general — e.g. if promoting \"Masala Dosa\" at an Indian Restaurant, the hero_element should be the dosa itself, not generic Indian food." : '') . buildSpecialInstructionsBlock($additional);
...
<div class="card">
    ...
    <form method="POST">
        ...
        <?php if ($company): ?>
        <div style="margin-top:12px;border:1.5px solid var(--border);border-radius:10px;padding:12px 14px;background:var(--bg);" id="userMediaCard">
            <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                <span style="font-size:13px;font-weight:600;color:var(--dark-blue);">📸 Use My Images/Videos</span>
                <button type="button" class="btn btn-gray" style="margin-left:auto;font-size:12px;padding:7px 14px;" onclick="document.getElementById('userMediaInput').click()">+ Upload</button>
            </div>
            <div style="font-size:11px;color:var(--muted);margin-top:5px;">Upload your own photos or clips of this item — they're tagged and added to your library so future scenes can reuse them instead of generating a new AI image.</div>
            <input type="file" id="userMediaInput" accept="image/*,video/*" multiple style="display:none;" onchange="handleUserMediaUpload(this)">
            <div id="userMediaResults" style="margin-top:10px;display:flex;flex-wrap:wrap;gap:8px;"></div>
        </div>
        <?php endif; ?>
        <div style="margin-top:12px;">
...
<?php endif; ?>
<script>
async function handleUserMediaUpload(input) {
    const files = Array.from(input.files || []);
    if (!files.length) return;
    const results = document.getElementById('userMediaResults');
    ...
    for (const file of files) {
        ...
        slot.style = 'width:96px;border:1px solid var(--border);border-radius:8px;overflow:hidden;background:var(--card);';

        const thumbWrap = document.createElement('div');
        thumbWrap.style = 'width:96px;height:96px;background:#000;display:flex;align-items:center;justify-content:center;overflow:hidden;';
        if (isVideo) {
        ...
        label.style = 'font-size:10px;color:var(--muted);padding:4px 6px;line-height:1.3;word-break:break-word;';
        ...
        try {
            ...
            if (data.success) {
                ...
                label.textContent = '✓ ' + (data.tagged ? 'Tagged' : 'Saved');
            } else {
                label.style.color = '#dc2626';
                label.textContent = '✕ ' + (data.error || data.message || 'Failed');
            }
        } catch (e) {
            label.style.color = '#dc2626';
    ...
    <div class="card">
        ...
        <div id="videoCostCard" style="display:none; margin-top:20px;">
            <div style="border:1.5px solid var(--border);border-radius:14px;padding:18px 20px;background:#fff;">
                <div style="font-size:13px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;margin-bottom:12px;">🎬 Next Step — Choose Video Type</div>
                <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;margin-bottom:16px;">
                    <div style="font-size:28px;">⚡</div>
                    <div style="flex:1;">
                        <div style="font-size:15px;font-weight:800;color:#0f2a44;" id="videoCostLine">Calculating cost…</div>
                        <div style="font-size:11px;color:#64748b;margin-top:3px;" id="videoCostDetail">Checking your media library for existing videos…</div>
                    </div>
                    <div style="background:#f59e0b;color:#fff;font-size:13px;font-weight:800;padding:4px 14px;border-radius:20px;white-space:nowrap;" id="videoCostBadge">— credits</div>
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                    ...
                    <button type="button" onclick="generateAllVideos()" id="btnGenerateVideos"
                        ...
                        <div style="font-size:11px;color:#92400e;line-height:1.5;margin-top:4px;" id="aiVideoBtnDesc">Generate a real AI video clip for each scene via fal.ai. Renders in the background.</div>
                    </button>
                </div>
...
async function generateAllScenes() {
    ...
    function renderCard(sc, data) {
        ...
        if (!card) return;
        if (data && data.success && data.image_ready && data.image_folder && data.image_file) {
            // Image/video is already generated/matched and on disk — show it now, no polling needed.
            const imgPath = data.image_folder + '/' + data.image_file + '?t=' + Date.now();
            const isVideo = /\.(mp4|mov|webm|m4v)$/i.test(data.image_file);
            ...
            const badge = data.from_library
                ? '<div style="position:absolute;top:5px;left:5px;padding:3px 8px;border-radius:5px;font-size:10px;font-weight:700;background:rgba(99,102,241,.88);color:#fff;">📚 From library</div>'
            ...
            const storyIdForVideo = storyIdMap[String(sc.num)] || storyIdMap[sc.num] || 0;
            card.innerHTML = `
            ...
            // that image generation is synchronous, but kept as a safe fallback).
            card.innerHTML = `
                <div style="aspect-ratio:9/16;background:linear-gradient(135deg,#f0fdf4,#dcfce7);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:16px;gap:8px;">
                    <div style="font-size:28px;">⏳</div>
                    <div style="font-size:12px;font-weight:800;color:#166534;text-align:center;">Scene ${sc.num} processing</div>
            ...
            generatedScenes.push({ num:sc.num, wan:sc.wan, caption:sc.caption, file:'', source:'pending' });
        } else {
    ...
    async function genScene(sc) {
        ...
        try {
            ...
            if (data.success) {
                successCount++;
                // Reveal cost card on the first successful image so it
                // appears below the storyboard as soon as something is shown.
                // Use scenes.length from the outer scope for the total count.
                if (successCount === 1) {
                    revealVideoCostCard([], scenes.length);
                }
...
function revealVideoCostCard(imgScenes, totalScenes) {
    ...
    const costLine   = document.getElementById('videoCostLine');
    const costDetail = document.getElementById('videoCostDetail');
    const costBadge  = document.getElementById('videoCostBadge');
    const costBadgeBtn = document.getElementById('videoCostBadgeBtn');

    if (videoMatches > 0) {
        costLine.textContent   = creditCost + ' credits — ' + needGeneration + ' video' + (needGeneration !== 1 ? 's' : '') + ' to generate';
        costDetail.textContent = videoMatches + ' scene' + (videoMatches !== 1 ? 's' : '') + ' already have a matching video in your library (free). '
                               + needGeneration + ' scene' + (needGeneration !== 1 ? 's' : '') + ' need generation via fal.ai at 20 credits each.';
    } else {
        costLine.textContent   = creditCost + ' credits — ' + needGeneration + ' video' + (needGeneration !== 1 ? 's' : '') + ' via fal.ai';
        costDetail.textContent = needGeneration + ' scene' + (needGeneration !== 1 ? 's need' : ' needs') + ' video generation at 20 credits each. Minimum 20 credits applies.';
    }
    if (costBadge)    costBadge.textContent    = creditCost + ' cr';
...
// ── Trigger video generation — shell_exec cron launch to be wired in later ──
function generateAllVideos() {
    // TODO: fire shell_exec cron job via AJAX to process hdb_video_gen_que
    ...
    if (label) label.textContent = 'Videos Queued ⏳';
    if (desc)  desc.textContent  = 'Generating in the background via fal.ai…';
