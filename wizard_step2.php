<?php
	...
	// Ensure promo columns exist on hdb_image_data
	if ($conn) {
		mysqli_query($conn, "ALTER TABLE hdb_image_data ADD COLUMN IF NOT EXISTS promo_group    VARCHAR(200) DEFAULT NULL");
		mysqli_query($conn, "ALTER TABLE hdb_image_data ADD COLUMN IF NOT EXISTS promo_subgroup VARCHAR(200) DEFAULT NULL");
		mysqli_query($conn, "ALTER TABLE hdb_image_data ADD COLUMN IF NOT EXISTS skip_embedding TINYINT(1)   DEFAULT 0");
	}
	...
	function generateVoiceOpenAI_wiz($text, $voice_id, $filepath, $rate = '1.0') {
		...
		curl_setopt_array($ch, [
			...
			CURLOPT_TIMEOUT        => 10,   // 10s — JS fires all in parallel, retries failures after pipeline completes
		]);
		$_t = microtime(true);
		...
		if ($bytes === false || $bytes === 0) {
			...
			wiz_log("audio WRITE FAIL: $why | file=$filepath");
	...
	// ══════════════════════════════════════════════════════════════════════════
	// FUNCTION: vv_build_hashtags_keywords
	// Builds podcast-level hashtags/keywords from: title, ai_group, ai_subgroup,
	// target_audience, target_location. Audience/location are skipped when they
	// are 'Global' / 'General Public' (i.e. no specific targeting set).
	// ══════════════════════════════════════════════════════════════════════════
	if (!function_exists('vv_build_hashtags_keywords')) {
	function vv_build_hashtags_keywords($title, $ai_group, $ai_subgroup, $target_audience, $target_location) {
		$stop = ['the','and','for','you','your','with','that','this','are','can','will','have',
				 ...
				 'like','into','over','also','after','other','only','a','an','of','to','in','on'];
		...
		$title_words = preg_split('/\s+/', strtolower(preg_replace('/[^a-zA-Z0-9 ]/', ' ', (string)$title)));
		$title_words = array_values(array_filter($title_words, function($w) use ($stop) {
			return strlen($w) > 2 && !in_array($w, $stop);
		}));
		$title_words = array_slice(array_values(array_unique($title_words)), 0, 8);

		$aud_lower = strtolower(trim((string)$target_audience));
		$loc_lower = strtolower(trim((string)$target_location));
		$is_global_aud = ($aud_lower === '' || $aud_lower === 'global' || $aud_lower === 'general public');
		$is_global_loc = ($loc_lower === '' || $loc_lower === 'global');

		// ── Keywords ──
		$kw = $title_words;
		if ($ai_group)    $kw[] = strtolower($ai_group);
		if ($ai_subgroup) $kw[] = strtolower($ai_subgroup);
		if (!$is_global_aud) $kw[] = $aud_lower;
		if (!$is_global_loc) $kw[] = $loc_lower;
		$kw = array_values(array_unique(array_filter($kw, fn($w) => $w !== '')));
		$keywords = implode(', ', $kw);

		// ── Hashtags ──
		$ht = [];
		foreach ($title_words as $w) { $h = $slug($w); if ($h) $ht[] = '#' . $h; }
		if ($ai_group)    { $h = $slug($ai_group);    if ($h) $ht[] = '#' . $h; }
		if ($ai_subgroup) { $h = $slug($ai_subgroup); if ($h) $ht[] = '#' . $h; }
		if (!$is_global_aud) { $h = $slug($target_audience); if ($h) $ht[] = '#' . $h; }
		if (!$is_global_loc) { $h = $slug($target_location); if ($h) $ht[] = '#' . $h; }
		$ht = array_values(array_unique($ht));
		$hashtags = implode(' ', $ht);
	...
	function buildCaptionRows($conn, $podcast_id, $story_id, $scene_text, $user_settings_all, $is_broll = false) 
	{
		...
		if ((int)($cap['is_enabled'] ?? 1)) {
			...
			$py_cap = $is_broll ? 10 : -1;
			$fs_cap = $is_broll ? 20  : -1;
			if ($is_broll) {
				$cap['_anim_style'] = 'none';
				$cap['_anim_speed'] = 0.5;
				wiz_log("buildCaptionRows B-ROLL OVERRIDE APPLIED: _anim_style=none _anim_speed=0.5 position_y=$py_cap");
			}
			...
			if ($is_broll) {
				// B-Roll has a single scene holding the FULL script — never truncate.
				$caption_text = $clean_for_cap;
				wiz_log("buildCaptionRows B-ROLL: using FULL scene text for caption (" . strlen($caption_text) . " chars), no truncation");
			} elseif (preg_match('/^(.+?[.!?])\s+\S/u', $clean_for_cap, $m)) {
				// Multi-scene videos: take first sentence only
	...
	if ($action === 'search_images_batch') {
		...
		$company_id_bat = (int)($_POST['company_id']    ?? $_SESSION['company_id'] ?? 0);
		...
		// ── Load promo_group + promo_subgroup from hdb_companies ────────────────
		$promo_group    = '';
		$promo_subgroup = '';
		if ($company_id_bat > 0) {
			$cg = mysqli_fetch_assoc(mysqli_query($conn,
				"SELECT ai_group, ai_subgroup FROM hdb_companies WHERE id=$company_id_bat LIMIT 1"));
			if ($cg) {
		...
		wiz_log("promo_group='$promo_group' promo_subgroup='$promo_subgroup' company=$company_id_bat scenes=" . count($scenes_input) . " media=$media_type_filter");

		// ── Clause builders ─────────────────────────────────────────────────────
		// Subgroup clause — most specific (e.g. 'Bridal wear')
		$ps_clause = $promo_subgroup
			? "AND promo_subgroup IN ('$ps_esc','$ps_lc')"
			: '';
		// Group clause — broader fallback (e.g. 'Fashion & Beauty')
		$pg_clause = $promo_group
			? "AND promo_group IN ('$pg_esc','$pg_lc')"
			: '';

		$media_clause = $media_type_filter
			? "AND media_type='" . mysqli_real_escape_string($conn, $media_type_filter) . "'"
			: '';

		// ── Asset loader helper ──────────────────────────────────────────────────
		if (!function_exists('wiz_load_assets')) {
			function wiz_load_assets($conn, $company_id, $group_clause, $sub_clause, $media_clause, $podcast_id) {
				$excl = $podcast_id > 0
				...
				$sql = "SELECT id, image_name, natural_language_tags, media_type, embedding, thumbnail, image_folder
						...
						WHERE company_id=$company_id
						  ...
						  $group_clause $sub_clause $media_clause $excl
				...
				wiz_log("wiz_load_assets: company=$company_id group_clause='$group_clause' sub_clause='$sub_clause' rows=" . count($assets) . " ms=" . round((microtime(true)-$t0)*1000));
				return $assets;
			}
		}

		// ── Asset loading — same filter for all tiers, different media type ────────
		// Tier 1: company assets + subgroup + VIDEO
		// Tier 2: company assets + subgroup + IMAGE  
		// Tier 3: shared stock (company_id=0) + subgroup + VIDEO
		// Tier 4: AI generate (no assets needed — flagged per scene)

		$video_clause = "AND media_type='video'";
		$image_clause = "AND media_type='image'";

		// Company videos — Bridal wear
		$co_videos = $company_id_bat > 0
			? wiz_load_assets($conn, $company_id_bat, $pg_clause, $ps_clause, $video_clause, $podcast_id)
			: [];

		// Company images — Bridal wear
		$co_images = $company_id_bat > 0
			? wiz_load_assets($conn, $company_id_bat, $pg_clause, $ps_clause, $image_clause, $podcast_id)
			: [];

		// Shared stock videos — Bridal wear
		$shared_videos = wiz_load_assets($conn, 0, $pg_clause, $ps_clause, $video_clause, $podcast_id);

		// Shared stock images — Bridal wear (fallback before generate)
		$shared_images = wiz_load_assets($conn, 0, $pg_clause, $ps_clause, $image_clause, $podcast_id);

		wiz_log("assets: co_videos=" . count($co_videos) . " co_images=" . count($co_images) . " shared_videos=" . count($shared_videos) . " shared_images=" . count($shared_images) . " load_time=" . round((microtime(true)-$_t_batch)*1000) . "ms");

		if (count($co_videos) + count($co_images) + count($shared_videos) + count($shared_images) === 0) {
			wiz_log("WARNING: No assets found for promo_group='$promo_group' promo_subgroup='$promo_subgroup' — will generate AI images for all scenes");
		}
			...
			$recent_res = mysqli_query($conn,
				"SELECT id FROM hdb_podcasts
				 WHERE (admin_id=$scope_id OR team_lead_id=$scope_id)
				   AND id!=$podcast_id AND video_status NOT IN ('ARCHIVED')
				   AND (archived_flag IS NULL OR archived_flag=0)
				 ORDER BY id DESC LIMIT 10");
			$recent_ids = [];
			if ($recent_res) while ($r = mysqli_fetch_assoc($recent_res)) $recent_ids[] = (int)$r['id'];
			if (!empty($recent_ids)) {
				...
				if ($used_res) while ($r = mysqli_fetch_assoc($used_res)) $recently_used[] = $r['image_file'];
		...
		foreach ($scenes_input as $idx => $scene) {
			...
			$search_query = !empty($nl_tags) ? $nl_tags : $query;

			// Get embedding — reuse cached if same query
			$cache_key = md5(strtolower($search_query));
			if (!isset($embedding_cache[$cache_key])) {
				$embedding_cache[$cache_key] = function_exists('getEmbeddingForSearch')
					? getEmbeddingForSearch($search_query, $apiKey)
					: null;
			}
			$vec = $embedding_cache[$cache_key];

			if (!$vec) {
				wiz_log("embedding failed for scene $idx — skipping (no fallback to unfiltered search)");
				$results[] = ['scene_idx'=>$idx, 'found'=>[]];
				continue;
			}
			...
			// Score each pool separately using cosine similarity
			function wiz_score_pool($pool, $vec, $dims, $tier) {
				$scored = [];
			...
			$pool1 = wiz_score_pool($co_videos,     $vec, $dims, 1); // company videos
			$pool2 = wiz_score_pool($co_images,     $vec, $dims, 2); // company images
			$pool3 = wiz_score_pool($shared_videos, $vec, $dims, 3); // shared videos
			$pool4 = wiz_score_pool($shared_images, $vec, $dims, 4); // shared images

			// Pick from pools in order — first non-empty pool wins
			$assigned = [];
			$chosen_pool = 'none';

			foreach ([$pool1, $pool2, $pool3, $pool4] as $pi => $pool) {
				if (empty($pool)) continue;
				foreach ($pool as $m) {
					if (count($assigned) >= $slots_needed) break;
					if (in_array($m['filename'], $used_filenames)) continue;
					if (in_array($m['filename'], $recently_used))  continue;
					$assigned[] = $m;
					$used_filenames[] = $m['filename'];
					if ($podcast_id > 0) {
						$fn_e = mysqli_real_escape_string($conn, $m['filename']);
						mysqli_query($conn, "INSERT IGNORE INTO hdb_build_used_files (podcast_id,filename) VALUES ($podcast_id,'$fn_e')");
					}
				}
				if (!empty($assigned)) { $chosen_pool = 'tier' . ($pi+1); break; }
			}

			// Tier 4: AI generate — nothing found in any pool
			if (empty($assigned)) {
				wiz_log("scene $idx: no assets found in any tier — flagging for AI generation | query=" . substr($search_query,0,60));
				$assigned[] = [
					'filename'     => '',
			...
			wiz_log("scene $idx: pool1=" . count($pool1) . " pool2=" . count($pool2) . " pool3=" . count($pool3) . " pool4=" . count($pool4) . " chosen=$chosen_pool assigned=" . count($assigned) . " time=" . wiz_ms($_t_scene));
			$results[] = ['scene_idx'=>$idx, 'found'=>$assigned];
		}

		wiz_log("=== SEARCH_IMAGES_BATCH DONE | scenes=" . count($results) . " total=" . wiz_ms($_t_batch) . " ===");
	...
	if ($action === 'create_scenes_from_content') {
		try {
			...
			// Preserve the real video_type (e.g. 'broll') — do NOT force 'standard'.
			$reel_type_esc = mysqli_real_escape_string($conn, $reel_type);
			mysqli_query($conn,
				"UPDATE hdb_podcasts SET video_type='$reel_type_esc', updated_at=NOW() WHERE id=$podcast_id");
			wiz_log("create_scenes_from_content: kept video_type=$reel_type_esc for podcast=$podcast_id");
			echo json_encode([
				'success'     => true,
...
// ══════════════════════════════════════════════════════════════════════════════
	if ($action === 'create_scenes_from_podcast') {
		...
		try {
			...
			// Target audience / location — default to 'Global' when nothing is set
			$target_audience = trim($_POST['target_audience'] ?? $pod['target_audience'] ?? '');
			$target_location = trim($_POST['target_location'] ?? $pod['target_location'] ?? '');
			if ($target_audience === '') $target_audience = 'Global';
			...
			[$pod_hashtags, $pod_keywords] = vv_build_hashtags_keywords($title, $ai_group, $ai_subgroup, $target_audience, $target_location);
			$pod_hashtags_esc = mysqli_real_escape_string($conn, $pod_hashtags);
			$pod_keywords_esc = mysqli_real_escape_string($conn, $pod_keywords);
			wiz_log("create_scenes_from_podcast: hashtags=$pod_hashtags_esc | keywords=$pod_keywords_esc");

			mysqli_query($conn,
				"UPDATE hdb_podcasts SET internal_status='scenes_ready', video_type='$reel_type', updated_at=NOW(),
				 ai_group='$ai_group', ai_subgroup='$ai_subgroup',
				 hashtags='$pod_hashtags_esc', keywords='$pod_keywords_esc'
				 WHERE id=$podcast_id");
	...
	if ($action === 'generate_scene_audio') {
		...
		if ($result['success']) {
			...
			// Guard: even if the writer reported success, confirm the file is really on disk
			$fsize = (file_exists($filepath)) ? (int)filesize($filepath) : 0;
			if ($fsize === 0) {
				wiz_log("⏱ audio FAIL scene=$scene_id elapsed=" . wiz_sec($_t_audio) . " err=audio file empty/missing after write");
				if (ob_get_level()) ob_clean(); // drop any leaked PHP warning so response is pure JSON
				echo json_encode(['success'=>false,'error'=>'Audio file was not saved (empty or missing on server)']);
				if (defined('WIZARD_INCLUDED') && WIZARD_INCLUDED) return; else exit;
			}
...
// ══════════════════════════════════════════════════════════════════════════════
	if ($action === 'assign_image') {
		...
		// Look up thumbnail, nl_tags AND owner from hdb_image_data
		$thumb         = '';
		$asset_nl_tags = '';
		$asset_admin   = 0;
		$asset_company = 0;
		...
		$tq = mysqli_query($conn,
			"SELECT thumbnail, natural_language_tags, media_type as db_media_type, admin_id, company_id FROM hdb_image_data WHERE image_name='$filename' LIMIT 1");
		if ($tq && $tr = mysqli_fetch_assoc($tq)) {
			...
			$asset_admin   = (int)($tr['admin_id']   ?? 0);
			$asset_company = (int)($tr['company_id'] ?? 0);
		}
		wiz_log("⏱ assign_image lookup: " . wiz_ms($_t_lookup) . " | scene=$scene_id slot=$slot_number file=$filename asset_admin=$asset_admin asset_company=$asset_company");
		...
		// Company asset (Tier 1 or 2) → user_id_X_company_id_Y/podcast_images or podcast_videos
		// Shared stock (Tier 3) → podcast_videos / podcast_images
		if ($asset_admin > 0 && $asset_company > 0) {
			$base_folder  = 'user_id_' . $asset_admin . '_company_id_' . $asset_company;
			$image_folder = $base_folder . '/' . ($is_video ? 'podcast_videos' : 'podcast_images');
		} else {
			$image_folder = $is_video ? 'podcast_videos' : 'podcast_images';
		}
		wiz_log("assign_image media detect: file=$filename ext=$file_ext is_video=" . ($is_video?'YES':'NO') . " image_folder=$image_folder asset_admin=$asset_admin asset_company=$asset_company");
...
// Replace your existing 'get_thumbnail_from_scene' action handler with this:
	...
	if ($action === 'search_images') {
		...
		$company_id_si   = (int)($_POST['company_id'] ?? $_SESSION['company_id'] ?? 0);
		...
		$results = searchAssets($conn, $query, $apiKey, $podcast_id, $media_type, $include_mine, $admin_id, $limit, 0.25, $company_id_si);
	...
	if ($action === 'generate_image_api') {
		...
		$company_id_bat  = (int)($_POST['company_id'] ?? $_SESSION['company_id'] ?? $company_id ?? 0);

		// Get ai_group + ai_subgroup from company for tagging
		$ai_group_val = ''; $ai_sub_val = '';
		if ($company_id_bat > 0) {
			$co_tag = mysqli_fetch_assoc(mysqli_query($conn,
				"SELECT ai_group, ai_subgroup FROM hdb_companies WHERE id=$company_id_bat LIMIT 1"));
			if ($co_tag) { $ai_group_val = trim($co_tag['ai_group'] ?? ''); $ai_sub_val = trim($co_tag['ai_subgroup'] ?? ''); }
		}
		$ai_grp_e = mysqli_real_escape_string($conn, $ai_group_val);
		$ai_sub_e = mysqli_real_escape_string($conn, $ai_sub_val);
		...
		// AI generated images always go to shared stock (admin_id=0, company_id=0)
		...
		// ── 3. Hand off to mediaIngest() — thumbnail + DB insert + vision tag + embedding ──
		if (!defined('MEDIA_INGEST_LOADED')) require_once __DIR__ . '/media_ingest.php';

		$context_str = $ai_group_val
			? "This image was AI-generated for a '{$ai_group_val}' business" . ($ai_sub_val ? " in the '{$ai_sub_val}' niche" : '') . '.'
			: 'AI-generated stock image for video production.';

		$ingest = mediaIngest([
			'local_path'      => $result['filepath'],   // already saved by Flux/OpenAI
			'admin_id'        => 0,                     // shared stock
			'company_id'      => 0,                     // shared stock
			'promo_group'     => $ai_group_val,
			'promo_subgroup'  => $ai_sub_val,
			'context'         => $context_str,
			'filename_prefix' => 'generated_' . $podcast_id . '_' . str_pad($seq_no, 3, '0', STR_PAD_LEFT),
			'max_video_sec'   => 0,                     // images — no trim needed
			'skip_tagging'    => false,
			'skip_if_exists'  => false,
			'root_dir'        => __DIR__,
		], $conn, $apiKey);

		wiz_log("generate_image_api: mediaIngest result=" . json_encode(array_intersect_key($ingest, array_flip(['success','image_id','filename','folder','tagged','embedding_dims','message']))));

		if (!$ingest['success']) {
			echo json_encode(['success' => false, 'error' => $ingest['message']]);
			if (defined('WIZARD_INCLUDED') && WIZARD_INCLUDED) return; else exit;
		}

		$filename      = $ingest['filename'];
		$image_folder  = $ingest['folder'];       // 'podcast_images'
		$thumbFilename = $ingest['thumbnail'];    // set by mediaIngest
		$image_id      = $ingest['image_id'];
		$esc_filename  = mysqli_real_escape_string($conn, $filename);
		$esc_thumbname = mysqli_real_escape_string($conn, $thumbFilename);
		wiz_log("generate_image_api: file=$filename folder=$image_folder thumb=$thumbFilename id=$image_id tagged=" . ($ingest['tagged']?'YES':'NO') . " dims=" . $ingest['embedding_dims']);
