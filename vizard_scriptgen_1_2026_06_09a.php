...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'get_company_video_ideas') {
    ...
    if (!$niche_name) { echo json_encode(['success'=>false,'error'=>'Missing niche_name']); exit; }

    // Step 1: Check hdb_user_video_ideas for this user + niche
    $ideas = [];
    $q = mysqli_query($conn,
    ...
    vv_log("get_company_video_ideas | niche=$niche_name admin=$owner_id found=" . count($ideas));

    // Step 2: If nothing found — call AI, show and save
    if (empty($ideas)) {
        require_once __DIR__ . '/config.php';
        $apiKey_fb = $apiKey ?? $chatgpt_api_key ?? '';
        $focus     = $niche_name;
        $context   = $industry ? "$industry > $niche_name" : $niche_name;
        $fb_prompt = "You are a video content strategist. Generate 10 video TOPIC ideas for: \"$context\"\n"
                   ...
                   . "Return ONLY a JSON array of 10 strings. No markdown.";
        $ch = curl_init('https://api.openai.com/v1/chat/completions');
        ...
        if ($fb_resp) {
            ...
            if (is_array($fb_items) && count($fb_items) > 0) {
                $ind_esc = mysqli_real_escape_string($conn, $industry ?: $niche_name);
                $saved   = 0;
                foreach (array_slice($fb_items, 0, 10) as $idea) {
                    ...
                    $ok = mysqli_query($conn,
                        "INSERT INTO hdb_user_video_ideas
                            (admin_id,company_id,niche_id,category_id,niche_name,category_name,video_idea,is_ai_generated)
                         VALUES ($owner_id,$co_id,0,0,'$niche_name','$ind_esc','$ie',1)");
                    if ($ok) $saved++;
                    else vv_log("get_company_video_ideas INSERT failed: " . mysqli_error($conn) . " | idea=$idea");
                }
                vv_log("get_company_video_ideas AI: generated=" . count($ideas) . " saved=$saved niche=$niche_name cat=$ind_esc admin=$owner_id co=$co_id");
            }
        }
    }
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'generate_company_video_ideas') {
    ...
    if ($httpCode === 200) {
        ...
        if (is_array($ideas) && count($ideas) > 0) {
            ...
            try {
                [$owner_id, $co_id] = vv_resolve_user($conn, $admin_id, $company_id);
                $niche_esc    = mysqli_real_escape_string($conn, $niche    ?: $subgroup);
                $category_esc = mysqli_real_escape_string($conn, $subgroup ?: $group);
                ...
                foreach (array_slice($ideas, 0, 10) as $idea) {
                    ...
                    $r1 = mysqli_query($conn, "INSERT IGNORE INTO hdb_user_video_ideas (admin_id,company_id,niche_id,category_id,niche_name,category_name,video_idea,is_ai_generated) VALUES ($owner_id,$co_id,0,0,'$niche_esc','$category_esc','$ie',1)");
...
async function renderVideoIdeaSelect(s) {
    ...
    const nicheName    = coIndustry.niche    || ans.niche          || ''; // ai_subgroup
    const categoryName = coIndustry.subgroup || ans.industry_desc  || ''; // ai_group
