...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Restore subtab from URL param (e.g. after page reload on topic save)
    const urlParams = new URLSearchParams(window.location.search);
    const subtab = urlParams.get('subtab');
    if (subtab === 'bank') {
        setTimeout(() => switchIdeaSubtab('bank'), 100);
...
async function assignImagesToAllScenes(podcastId, mediaType = 'stock_images') {
    ...
    let videosAssigned = 0, imagesAssigned = 0, generatedAssigned = 0;
    ...
    for (let i = 0; i < scenes.length; i++) {
        ...
        if (!scene.hashtags) {
            logToModal(`Scene ${sceneNumber}: no hashtags, skipping`, 'warning');
            completed++;
            continue;
        }
