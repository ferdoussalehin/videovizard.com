...
function vv_build_hashtags_keywords($conn, $all_caption_text, $biz_part, $loc_part, $aud_part) {
    ...
    // Keywords come from the actual video content (scene captions) — these
    // are the only words that genuinely describe what's IN this specific
    // video. Business category / location / audience are each added as ONE
    // clean tag below, not also exploded into individual words — exploding
    // them was the source of duplicate noise like "milton" + "halton" +
    // "ontario" + "canada" + the whole phrase again as a separate fragment.
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'create_podcast_from_step1') {
    ...
    if (!$image_file) {
        echo json_encode(['success'=>false,'message'=>'Generate the model image in Step 1 first']); exit;
    }
    $protocol     = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $host         = $_SERVER['HTTP_HOST'];
    $doc_root     = rtrim($_SERVER['DOCUMENT_ROOT'], '/');
    ...
    echo json_encode([
        ...
        'estimated_cost'    => count($scenes_out) * 20, // 20cr/scene — charged later at Step 5 firing, not here
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'set_video_mode') {
    ...
    if (!$count_row || (int)$count_row['c'] === 0) {
        echo json_encode(['success'=>false,'message'=>'No scenes found for this podcast — build video scenes (Step 4) first']); exit;
    }
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'create_podcast_from_poses') {
    ...
    $esc_title  = mysqli_real_escape_string($conn, $title);
    $lang_code  = 'en';
    $reel_type  = 'standard';                    // ASSUMPTION — adjust if fashion videos need a different video_type/reel_type
    ...
    <div class="wiz-body">
      ...
      <div id="garmentStep1Wrap" style="display:none;">
        ...
        <div class="settings-bar" onclick="toggleStep('step1')">
          <span class="settings-bar-label">Step 1 — Mannequin / Product → Model</span>
        ...
        <div id="garmentStep23Wrap" style="display:none;margin-top:16px;">
          ...
          <div id="step2Wrap" style="margin-top:16px;">
            <div class="settings-bar" onclick="toggleStep('step2')">
              <span class="settings-bar-label">Step 2 — Select a Style</span>
              <span id="step2Summary" class="settings-bar-summary"></span>
              <span class="settings-bar-edit" id="step2Chevron">▾</span>
            </div>
            <div id="step2Body">
              <div class="garment-subheading">Pick a pose sequence. The model + garment image generated in Step 1 is reused across every scene in the sequence.</div>
              <div id="styleNeedsModelHint" style="display:none;font-size:12px;color:#dc2626;font-weight:600;margin-bottom:8px;">⚠ Generate the model in Step 1 above before picking a style.</div>
              <div id="styleGrid" class="style-grid"></div>
            </div>
          </div>

          <!-- ── Step 3 — Background Theme (optional) ──────────────────────── -->
          <div id="step3Wrap" style="display:none;margin-top:16px;">
            <div class="settings-bar" onclick="toggleStep('step3')">
              <span class="settings-bar-label">Step 3 — Background Theme <span style="font-size:11px;font-weight:400;color:var(--muted);">(optional)</span></span>
          ...
          <!-- ── Step 6 — Select Video Mode (shown once poses are generated) ── -->
          <div id="step6Wrap" style="display:none;margin-top:16px;">
            <div class="garment-heading">Step 6 — Select Video Mode</div>
...
<!-- ── Credit / Upgrade modal (Step 6 — AI Motion Video gate) ─────────────── -->
...
// Collapses Step 1 and reveals Step 2 (style picker) — the model image
// generated above is reused as-is, no separate model-selection step needed.
function step1ContinueToStyle() {
    collapseStep('step1', document.getElementById('step1Title').value.trim() || 'model generated');
...
function styleSelect(styleId) {
    ...
    if (!step1Draft.generatedUrl) {
        showToast('Generate the model in Step 1 first');
        return;
    }
    selectedStyle = style;
