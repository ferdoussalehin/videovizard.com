...
function pod_log(string $msg): void {
    error_log('[wizard_step2_podcast] ' . $msg . PHP_EOL, 3, __DIR__ . '/a_errors.log');
}
...
// ── getUserSettingsWiz ────────────────────────────────────────────────────────
// Loads caption / header / footer / logo settings for this admin from the DB.
// Falls back to sensible defaults if no row exists, so buildCaptionRows()
// always receives a fully-populated array and never throws an undefined-key error.
function getUserSettingsWiz(mysqli $conn, int $admin_id): array {
    ...
    $default_caption = [
        ...
        'caption_position'=>'bottom','text_align_v'=>'bottom',
        'caption_text'=>'',
    ];
    $default_header = [
        ...
        'caption_position'=>'top','text_align_v'=>'top',
        'caption_text'=>'',
    ];
    $default_footer = [
    ...
    // ── Try to load from hdb_user_settings ───────────────────────────────
    $result = @mysqli_query($conn,
        "SELECT * FROM hdb_user_settings WHERE admin_id = $admin_id LIMIT 1");

    if (!$result || mysqli_num_rows($result) === 0) {
        pod_log("getUserSettingsWiz: no row for admin=$admin_id — using defaults");
        return [
    ...
    // ── Map DB row columns into the expected sub-arrays ───────────────────
    // Each key is prefixed by its type (cap_, hdr_, ftr_, logo_) in the DB.
    // If the table uses a different prefix or JSON blob, fall back gracefully.

    // Helper: read a DB key with fallback to default
    $g = fn($key, $default) => $row[$key] ?? $default;

    // Attempt JSON blob approach first (some installs store as JSON)
    if (!empty($row['caption_settings'])) {
        $cap_data = json_decode($row['caption_settings'], true) ?: [];
        $hdr_data = json_decode($row['header_settings']  ?? '{}', true) ?: [];
        $ftr_data = json_decode($row['footer_settings']  ?? '{}', true) ?: [];
        $logo_data= json_decode($row['logo_settings']    ?? '{}', true) ?: [];
        return [
            'caption' => array_merge($default_caption, $cap_data),
            'header'  => array_merge($default_header,  $hdr_data),
            'footer'  => array_merge($default_footer,  $ftr_data),
            'logo'    => array_merge($default_logo,    $logo_data),
        ];
    }

    // Flat column approach — map prefixed columns
    $caption = [
        'is_enabled'        => (int)$g('cap_is_enabled',    1),
        'fontfamily'        => $g('cap_fontfamily',         'Arial'),
        'fontsize'          => (int)$g('cap_fontsize',      28),
        'fontcolor'         => $g('cap_fontcolor',          '#ffff00'),
        'fontweight'        => $g('cap_fontweight',         'bold'),
        'font_italic'       => (int)$g('cap_font_italic',   0),
        'font_underline'    => (int)$g('cap_font_underline',0),
        'fontcolor_bg'      => $g('cap_fontcolor_bg',       '#000000'),
        'fontbg_enable'     => (int)$g('cap_fontbg_enable', 0),
        'caption_alignment' => $g('cap_alignment',          'center'),
        'position_x'        => (int)$g('cap_position_x',   50),
        'position_y'        => (int)$g('cap_position_y',   250),
        'width'             => (int)$g('cap_width',         500),
        'caption_speed'     => (float)$g('cap_speed',      1.0),
        'text_animation'    => $g('cap_text_animation',     'static'),
        'text_effect'       => $g('cap_text_effect',        'none'),
        'stroke_width'      => (int)$g('cap_stroke_width',  0),
        'stroke_color'      => $g('cap_stroke_color',       '#000000'),
        'shadow_color'      => $g('cap_shadow_color',       '#000000'),
        'gradient_color'    => $g('cap_gradient_color',     '#ff6600'),
        'display_mode'      => $g('cap_display_mode',       'full'),
        'caption_style'     => $g('cap_caption_style',      'none'),
        'caption_position'  => $g('cap_position',           'bottom'),
        'text_align_v'      => $g('cap_text_align_v',       'bottom'),
        'caption_text'      => $g('cap_caption_text',       ''),
    ];

    $header = [
        'is_enabled'        => (int)$g('hdr_is_enabled',    0),
        'fontfamily'        => $g('hdr_fontfamily',         'Helvetica'),
        'fontsize'          => (int)$g('hdr_fontsize',      16),
        'fontcolor'         => $g('hdr_fontcolor',          '#ffffff'),
        'fontweight'        => $g('hdr_fontweight',         'bold'),
        'font_italic'       => (int)$g('hdr_font_italic',   0),
        'font_underline'    => (int)$g('hdr_font_underline',0),
        'fontcolor_bg'      => $g('hdr_fontcolor_bg',       '#1a1a2e'),
        'fontbg_enable'     => (int)$g('hdr_fontbg_enable', 1),
        'caption_alignment' => $g('hdr_alignment',          'center'),
        'position_x'        => (int)$g('hdr_position_x',   0),
        'position_y'        => (int)$g('hdr_position_y',   0),
        'width'             => (int)$g('hdr_width',         1080),
        'caption_speed'     => (float)$g('hdr_speed',      1.0),
        'text_animation'    => $g('hdr_text_animation',     'fade_in'),
        'text_effect'       => $g('hdr_text_effect',        'none'),
        'stroke_width'      => (int)$g('hdr_stroke_width',  0),
        'stroke_color'      => $g('hdr_stroke_color',       '#000000'),
        'shadow_color'      => $g('hdr_shadow_color',       '#000000'),
        'gradient_color'    => $g('hdr_gradient_color',     '#ff6600'),
        'display_mode'      => $g('hdr_display_mode',       'full'),
        'caption_style'     => $g('hdr_caption_style',      'box'),
        'caption_position'  => $g('hdr_position',           'top'),
        'text_align_v'      => $g('hdr_text_align_v',       'top'),
        'caption_text'      => $g('hdr_caption_text',       ''),
    ];

    $footer = [
        'is_enabled'        => (int)$g('ftr_is_enabled',    0),
        'fontfamily'        => $g('ftr_fontfamily',         'Georgia'),
        'fontsize'          => (int)$g('ftr_fontsize',      12),
        'fontcolor'         => $g('ftr_fontcolor',          '#aaaaaa'),
        'fontweight'        => $g('ftr_fontweight',         'normal'),
        'font_italic'       => (int)$g('ftr_font_italic',   0),
        'font_underline'    => (int)$g('ftr_font_underline',0),
        'fontcolor_bg'      => $g('ftr_fontcolor_bg',       '#000000'),
        'fontbg_enable'     => (int)$g('ftr_fontbg_enable', 0),
        'caption_alignment' => $g('ftr_alignment',          'center'),
        'position_x'        => (int)$g('ftr_position_x',   0),
        'position_y'        => (int)$g('ftr_position_y',   0),
        'width'             => (int)$g('ftr_width',         1080),
        'caption_speed'     => (float)$g('ftr_speed',      1.0),
        'text_animation'    => $g('ftr_text_animation',     'static'),
        'text_effect'       => $g('ftr_text_effect',        'none'),
        'stroke_width'      => (int)$g('ftr_stroke_width',  0),
        'stroke_color'      => $g('ftr_stroke_color',       '#000000'),
        'shadow_color'      => $g('ftr_shadow_color',       '#000000'),
        'gradient_color'    => $g('ftr_gradient_color',     '#ff6600'),
        'display_mode'      => $g('ftr_display_mode',       'full'),
        'caption_style'     => $g('ftr_caption_style',      'none'),
        'caption_position'  => $g('ftr_position',           'bottom'),
        'text_align_v'      => $g('ftr_text_align_v',       'bottom'),
        'caption_text'      => $g('ftr_caption_text',       ''),
    ];

    $logo = [
        'logo_enabled'  => (int)$g('logo_enabled',    0),
        'logo_file'     => $g('logo_file',             ''),
        'logo_name'     => $g('logo_name',             ''),
        'logo_pos_h'    => $g('logo_pos_h',            'right'),
        'logo_pos_v'    => $g('logo_pos_v',            'top'),
        'logo_size_pct' => (int)$g('logo_size_pct',   15),
        'position_x'    => (int)$g('logo_position_x', 960),
        'position_y'    => (int)$g('logo_position_y', 20),
        'width'         => (int)$g('logo_width',       120),
    ];

    pod_log("getUserSettingsWiz: loaded settings for admin=$admin_id");
    return compact('caption', 'header', 'footer', 'logo');
}
...
if ($action === 'process_podcast_scene') {
    ...
    $image_folder    = $is_talking_head ? 'podcast_avatars' : 'podcast_hostguest_images';

    if (!$scene_id || !$podcast_id) {
        echo json_encode(['success' => false, 'error' => 'Missing scene_id or podcast_id']);
        exit;
    }
...
if ($action === 'check_audio_file') {
    ...
    echo json_encode(['success' => true, 'exists' => $exists, 'deleted' => $deleted]);
    exit;
}
...
if ($action === 'get_podcaster_images') {
    ...
    echo json_encode([
        'success'       => true,
        'host_images'   => $host_images,   // for host picker
        'guest_images'  => $guest_images,  // for guest picker
        // Legacy 'images' key — merged list for any old callers
...
if ($action === 'create_scenes_from_podcast') {
    ...
    try {
        ...
        if (empty($script_text)) {
            echo json_encode(['success' => false, 'error' => 'No script_text found in podcast record']);
            exit;
        }
        ...
        if (empty($lines)) {
            echo json_encode(['success' => false, 'error' => 'Script produced no lines after splitting']);
            exit;
        }
