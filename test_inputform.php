...
function enforceSceneBreaks(script) {
  ...
  return script
    ...
    .map(scene => {
      ...
      if (!trimmed) return '';
      // Strip any existing break tag to avoid doubling
...
async function generateScript(){
  ...
  try {
    ...
    const script = enforceSceneBreaks(d.script);
...
async function campRenderTitleSelect(){
  ...
  body.innerHTML=`
    <div class="camp-progress-bar">
      📊 <strong>${days} days</strong> × <strong>${postsPerDay}/day</strong> × <strong>${langs.length} language${langs.length>1?'s':''}</strong>
...
async function generateCampaign(){
  ...
  try {
    ...
    const results=(d.results||[]).map(item=>({
      ...item,
      script: enforceSceneBreaks(item.script||'')
    }));
...
Reformat this content as a ${reelType} short-form video script.
...
- Last scene must be: ${cta} <break time="200ms"/>
...
${cta} <break time="200ms"/>`;
  ...
  try {
    ...
    // Enforce scene break tags client-side as safety net
    const processedScript = enforceSceneBreaks(d.script);
