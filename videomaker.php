...
// Review button: only shown once a final MP4 has actually been generated —
// video_status flips to 'RECORDED' and video_filename is set by
// vps_ffmpeg_stitch.php right after a successful stitch.
$video_status   = trim($row['video_status']   ?? '');
$video_filename = trim($row['video_filename'] ?? '');
$has_published_video = ($video_status === 'RECORDED' && $video_filename !== '');

// Category / sub-category (e.g. "Fashion" / "Bridal Wear") — used as the
// default media-search query so results are relevant to the show's niche.
$ai_group      = trim($row['ai_group']    ?? '');
$ai_subgroup   = trim($row['ai_subgroup'] ?? '');
$podcast_niche = trim($row['niche']       ?? '');

// Some writers of image_folder (e.g. the podcast-generation pipeline) store
// it WITHOUT the 'user_media/' prefix — e.g. "user_id_34_company_id_29/podcast_images"
// — while videomaker.php's own upload/library-pick code always writes the
// full "user_media/user_id_X_company_id_Y/..." path and reads image_folder
// back out verbatim. Both point at the exact same folder on disk; only the
// prefixed form resolves correctly as a URL on its own. This normalizes
// either form so a scene image doesn't silently 404 just because of which
// step originally wrote the row. Flat top-level folders (podcast_images,
// podcast_videos, podcast_themes, etc.) are left alone — they really do
// live at the site root, not under user_media/.
function vv_resolve_media_folder($folder) {
    $f = trim(trim((string)$folder), '/');
    if ($f === '') return '';
    if (preg_match('#^(user|admin)_id_\d+_company_id_\d+(/|$)#', $f) && strpos($f, 'user_media/') !== 0) {
        $f = 'user_media/' . $f;
    }
    return $f;
}

$img_folder  = vv_resolve_media_folder($row['image_folder'] ?? '') ?: 'podcast_images';
...
.vv-brand span.name span { color: #5fd1ff; }
.vv-back {
...
.vv-back:hover { background: rgba(255,255,255,0.22); color: #ffffff; }
...
/* video used to render scene into canvas — kept playing in the background
   at all times (see playVideoScene/toggleScenePlay) but visually hidden;
...
/* ── Side icon rails (left = main actions, right = sub-actions) ── */
.icon-rail {
    display: flex;
    flex-direction: column;
    gap: 8px;
...
.icon-rail-sub.empty { display: none; }
.icon-rail-group {
    display: flex;
    flex-direction: column;
    gap: 8px;
...
.scene-icon-btn.active { background: #dbeafe; border-color: var(--primary); color: var(--primary); }
...
/* ── Sub-action buttons (right rail) ── */
.scene-icon-btn.sub { background: var(--surface2); font-size: 9px; padding: 7px 4px; min-height: 54px; }
...
<div class="action-bar">
    <div class="action-bar-buttons">
        <button class="btn-action btn-review"   id="btnReview"   onclick="onReview()"
                style="<?= $has_published_video ? '' : 'display:none;' ?>">Review</button>
        <button class="btn-action btn-download" id="btnDownload" onclick="onDownloadTopBar()"
                style="<?= $has_published_video ? '' : 'display:none;' ?>">Download</button>
...
<div class="workspace">
    ...
    <!-- LEFT: main action icons -->
    <div class="icon-rail" id="mainIconRail">
        ...
        <!-- Tryon-only actions — only shown for video_type === 'tryon' podcasts.
             Left blank/unwired for now; functionality to be added next. -->
        <button class="scene-icon-btn" id="ibChangeModel" onclick="onChangeModel()" title="Change Model"
                style="<?= $video_type === 'tryon' ? '' : 'display:none;' ?>">
            <span class="icon-ico">🧍</span>
            <span>Model</span>
        </button>
        <button class="scene-icon-btn" id="ibChangeBg" onclick="onChangeBg()" title="Change Background"
                style="<?= $video_type === 'tryon' ? '' : 'display:none;' ?>">
            <span class="icon-ico">🌄</span>
    ...
    <!-- RIGHT: sub-action icons (one group shown at a time, based on which
         main action is active) — empty/hidden when none are open so the
         canvas stays centered. -->
    <div class="icon-rail-sub empty" id="subIconRail">

        <!-- Caption sub-icons -->
        <div class="icon-rail-group" id="capSubIcons" style="display:none;">
        ...
        <!-- Font sub-icons -->
        <div class="icon-rail-group" id="fontSubIcons" style="display:none;">
        ...
        <!-- Audio sub-icons -->
        <div class="icon-rail-group" id="audioSubIcons" style="display:none;">
...
<!-- ══ CHANGE MODEL OVERLAY (tryon videos only) ══ -->
<div class="cap-overlay" id="changeModelOverlay">
    <div class="cap-ov-panel" style="max-height:90vh;">
        <div class="cap-ov-head" style="background:#7c3aed;">
            <span>🧍 Change Model — Scene <span id="changeModelSceneNum">1</span></span>
            <button class="cap-ov-close" onclick="closeChangeModelOverlay()">✕</button>
        </div>
        <div class="cap-ov-body">

            <!-- Step A: model fields -->
            <div id="changeModelFormSection" class="cap-ov-section">
                <div class="cap-field-label" style="margin-bottom:6px;">Model — quick pick</div>
                <select id="cmPreset" class="cap-textarea" style="margin-bottom:10px;" onchange="_cmApplyPreset()">
                ...
                <div class="cap-field-label" style="margin-bottom:6px;">Model Description <span style="color:#dc2626;">*</span></div>
                <textarea id="cmDescription" class="cap-textarea" rows="3" style="margin-bottom:10px;"
                    placeholder="e.g. a Pakistani woman, around 25 years old, golden brown hair, brightly smiling, not looking directly at the camera"></textarea>

                <div class="cap-field-label" style="margin-bottom:6px;">Output Resolution</div>
                <select id="cmResolution" class="cap-textarea" style="margin-bottom:14px;">
                ...
                <button id="cmApplyBtn" style="width:100%;padding:12px;border-radius:9px;border:none;background:#7c3aed;color:#fff;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;" onclick="submitChangeModel()">
                    Apply New Model (5 cr)
                </button>
            </div>

            <!-- Step B: background picker — only shown if the person said "yes,
                 change the background too" after applying the model change -->
            <div id="changeModelThemeSection" class="cap-ov-section" style="display:none;">
                <div class="cap-field-label" style="margin-bottom:8px;">Pick a new background</div>
                <div id="changeModelThemeGrid" style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;"></div>
            </div>

            <div id="changeModelStatus" style="padding:0 4px 4px;font-size:12px;color:var(--muted);"></div>
        </div>
    </div>
</div>

<!-- ══ CHANGE BACKGROUND OVERLAY (tryon videos only) ══ -->
<div class="cap-overlay" id="changeBgOverlay">
    <div class="cap-ov-panel" style="max-height:90vh;">
        <div class="cap-ov-head" style="background:#7c3aed;">
            <span>🌄 Change Background — All Scenes</span>
            <button class="cap-ov-close" onclick="closeChangeBgOverlay()">✕</button>
        </div>
        <div class="cap-ov-body">
            <div class="cap-ov-section">
                <button style="width:100%;margin-bottom:10px;padding:11px;border-radius:9px;border:1.5px solid var(--border);background:var(--surface);color:var(--text);font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;" onclick="submitChangeBackground(null)">
                    🔀 Shuffle — new photos, same theme (all scenes)
                </button>
                <div class="cap-field-label" style="margin-bottom:8px;">...or pick a different theme</div>
                <div id="changeBgThemeGrid" style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;"></div>
            </div>
            <div id="changeBgStatus" style="padding:0 4px 4px;font-size:12px;color:var(--muted);"></div>
        </div>
    </div>
</div>
            ...
            <div style="display:flex;gap:6px;margin-bottom:8px;">
                ...
                <button onclick="performLibSearch()"
                    style="padding:7px 14px;border-radius:20px;border:none;background:var(--info);
                           color:#fff;font-size:11px;font-weight:700;cursor:pointer;white-space:nowrap;">🔍 Search</button>
            </div>
            <div style="display:flex;gap:4px;flex-wrap:wrap;">
                <button id="libTabVid"     onclick="setLibTab('video')"      class="lib-tab active">🎬 Videos <span id="libCountVid"></span></button>
                <button id="libTabImg"     onclick="setLibTab('image')"     class="lib-tab">🖼️ Images <span id="libCountImg"></span></button>
                <button id="libTabMineVid" onclick="setLibTab('mine_video')" class="lib-tab">👤 User Videos <span id="libCountMineVid"></span></button>
                <button id="libTabMineImg" onclick="setLibTab('mine_image')" class="lib-tab">👤 User Images <span id="libCountMineImg"></span></button>
            </div>
        </div>
...
<!-- ══ REVIEW MODAL — plays the generated published_videos/ MP4 ══ -->
<div id="reviewModal" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.92);
     align-items:center;justify-content:center;padding:16px;">
    <div style="position:relative;width:100%;max-width:420px;display:flex;flex-direction:column;align-items:center;">
        <button onclick="closeReviewModal()" aria-label="Close"
            style="position:absolute;top:-44px;right:0;background:rgba(255,255,255,.15);border:none;color:#fff;
                   width:34px;height:34px;border-radius:50%;cursor:pointer;font-size:18px;display:flex;
                   align-items:center;justify-content:center;">✕</button>
        <video id="reviewVideo" controls autoplay playsinline
            style="width:100%;max-height:80vh;border-radius:12px;background:#000;box-shadow:0 8px 40px rgba(0,0,0,.6);"></video>
    </div>
</div>
<style>
...
const USER_MEDIA_FOLDER        = 'user_media/admin_id_<?= (int)$admin_id ?>_company_id_<?= (int)$company_id ?>/';
const USER_MEDIA_FOLDER_IMAGES = USER_MEDIA_FOLDER + 'podcast_images/';
const USER_MEDIA_FOLDER_VIDEOS = USER_MEDIA_FOLDER + 'podcast_videos/';
const PODCAST_CATEGORY    = <?= json_encode($ai_group) ?>;
const PODCAST_SUBCATEGORY = <?= json_encode($ai_subgroup) ?>;
const PODCAST_NICHE       = <?= json_encode($podcast_niche) ?>;
let REVIEW_VIDEO_URL = <?= json_encode($has_published_video ? ('published_videos/' . $video_filename) : '') ?>;
const IMG_BASE     = <?= json_encode($img_folder.'/') ?>;
const FILE_FOLDER  = {};

// Mirrors vv_resolve_media_folder() on the PHP side — some scenes' image_folder
// is missing the 'user_media/' prefix that videomaker.php's own upload/library
// code always writes, which is why those images load fine via a direct URL
// (once you add the prefix yourself) but 404 inside the canvas. Flat
// top-level folders (podcast_images, podcast_videos, etc.) pass through as-is.
function resolveMediaFolder(rawFolder) {
    let f = (rawFolder || '').trim().replace(/^\/|\/$/g, '');
    if (!f) return '';
    if (/^(user|admin)_id_\d+_company_id_\d+(\/|$)/.test(f) && f.indexOf('user_media/') !== 0) {
        f = 'user_media/' + f;
    }
    return f;
}
...
(function() {
    SCENES.forEach(sc => {
        ...
        if (!fn) return;
        const f  = resolveMediaFolder(sc.image_folder);
...
function getSceneMediaFolder(sc) {
    const raw = resolveMediaFolder(sc.image_folder);
...
function mediaPath(scene) {
    ...
    const folder = resolveMediaFolder(scene.image_folder) || IMG_FOLDER || 'podcast_images';
    return fn ? folder + '/' + fn : null;
}

function thumbPath(scene) {
    const fn = (scene.image_file || '').trim();
    if (!fn) return null;
    const base = fn.replace(/\.[^.]+$/, '');
    return 'podcast_thumbnails/' + base + '_thumb.jpg';
}
...
let _videoThumbImg = null;

function playVideoScene(src, scene) {
    stopVideo();
    _videoThumbImg = null;
    sceneVideo.src = src;
    ...
    // Show the saved thumbnail immediately so there's no black screen while
    // the video buffers/loads in the background.
    const tPath = scene ? thumbPath(scene) : null;
    if (tPath) {
        loadImageCached(tPath).then(img => {
            _videoThumbImg = img;
            if (!_playing) {
            ...
            hideOverlay();
        }).catch(() => { /* no thumbnail on disk — fall back to video frame */ });
    }

    sceneVideo.addEventListener('canplay', function onFirstCanplay() {
        hideOverlay();
        if (!_videoThumbImg) { try { ctx.drawImage(sceneVideo, 0, 0, CW, CH); } catch(e) {} }
...
function onReview() {
    if (!REVIEW_VIDEO_URL) return;
    const v = document.getElementById('reviewVideo');
    if (v) v.src = REVIEW_VIDEO_URL + '?t=' + Date.now();
    document.getElementById('reviewModal').style.display = 'flex';
}

function onDownloadTopBar() {
    if (!REVIEW_VIDEO_URL) return;
    const a = document.createElement('a');
    a.href = REVIEW_VIDEO_URL;
    a.download = REVIEW_VIDEO_URL.split('/').pop();
...
async function onGenerate() {
    ...
    if (msg) msg.innerHTML = 'Encoding scenes · Burning captions · Mixing audio<br>Image scenes may take 1–3 min — please wait';
    ...
    const tick = setInterval(() => {
        ...
        if (s === 15 && msg) msg.innerHTML = 'Processing scenes · Applying pan &amp; zoom · Burning captions…<br>Almost there, please wait';
    ...
    try {
        ...
        if (!data.success) {
            console.error('Stitch failed — full response:', data);
            showGenerateError(data);
            if (btn) btn.disabled = false;
            return;
        }

        const filename = data.mp4_url ? data.mp4_url.split('/').pop() : ('podcast_' + PODCAST_ID + '.mp4');
        const mp4Url = data.mp4_url || ('published_videos/' + filename);
        // vps_ffmpeg_stitch.php already flipped video_status to 'RECORDED' and
        // saved video_filename — mirror that here so Review shows up immediately.
        REVIEW_VIDEO_URL = mp4Url;
        const reviewBtn = document.getElementById('btnReview');
        if (reviewBtn) reviewBtn.style.display = '';
        const downloadBtn = document.getElementById('btnDownload');
        if (downloadBtn) downloadBtn.style.display = '';
        openVidReadyModal(mp4Url, filename, data.mp4_size_mb || '?');

    } catch(e) {
        clearInterval(tick);
        if (ov) ov.style.display = 'none';
        showGenerateError({ error: 'Network error: ' + e.message });
    }
    if (btn) btn.disabled = false;
}

// ── "Video Ready" modal — simple 3-button version (Download / Schedule /
// Review), used in place of the full Publish Video modal for now. The full
// modal's own functions (openSchedModalWithMp4, vsDownload, etc.) are left
// below, untouched, in case it comes back later. ───────────────────────────
function openVidReadyModal(mp4Url, filename, sizeMb) {
    closeVidReadyModal();
    _vsRecURL = mp4Url; _vsRecFname = filename;
    window._mp4Ready = true; window._mp4Url = mp4Url; window._mp4Filename = filename;

    const savedEl = document.getElementById('vidReadyFilenameDisplay');
    if (savedEl) savedEl.innerHTML =
        `<div class="bpm-saved-dot"></div><span>Video ready — <strong>${filename}</strong> · ${sizeMb} MB ✅ MP4</span>`;

    const subEl = document.getElementById('vidReadySub');
    if (subEl) subEl.textContent = '<?= addslashes($row['title'] ?? 'Your Video') ?>';

    document.getElementById('vidReadyOverlay').classList.add('open');
}

function closeVidReadyModal() {
    document.getElementById('vidReadyOverlay')?.classList.remove('open');
}

function vidReadyDownload() {
    const a = document.createElement('a');
...
// Hands off to the dedicated scheduler page with enough context (podcast id +
// the actual video filename/URL just generated) that the person doesn't have
// to go re-pick the video over there.
function vidReadySchedule() {
    const params = new URLSearchParams({
        podcast_id:      PODCAST_ID,
        video_filename:  window._mp4Filename || _vsRecFname || '',
        video_url:       window._mp4Url || _vsRecURL || '',
    });
    window.location.href = 'vizard_scheduler.php?' + params.toString();
}

// Same player the existing Review button uses — just close this modal first
// so they're not stacked.
function vidReadyReview() {
    closeVidReadyModal();
    onReview();
}

// ── Schedule modal (full Publish Video version) ────────────────────────────────
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    document.getElementById('vidReadyOverlay')?.addEventListener('click',function(e){
        if(e.target===this)closeVidReadyModal();
    });
});
// ── Collapses the right-side sub-icon rail (keeps the canvas centered)
// whenever none of the 3 sub-groups (caption/font/audio) are open. ──────────
function _syncSubRail() {
    const anyOpen = ['capSubIcons','fontSubIcons','audioSubIcons']
        .some(id => document.getElementById(id)?.style.display === 'flex');
    document.getElementById('subIconRail')?.classList.toggle('empty', !anyOpen);
}
...
function onFont() {
    ...
    closeChangeModelOverlay(); closeChangeBgOverlay();
    if (open) {
        ...
        btn.classList.add('active');
        requestAnimationFrame(() => sub.scrollIntoView({ behavior: 'smooth', block: 'end' }));
    }
    _syncSubRail();
}
...
let _libImgs=[], _libVids=[], _libMineImgs=[], _libMineVids=[], _libSelectedFile=null, _libTab='video';
...
function onMedia() {
    ...
    document.getElementById('audioSubIcons').style.display='none';
    document.getElementById('ibAudio')?.classList.remove('active');
    closeCapOverlay(); closeFontOverlay(); closeAudioOverlay();
    closeChangeModelOverlay(); closeChangeBgOverlay();
    _syncSubRail();
    btn?.classList.add('active');
...
// ── Centered overlays are draggable by their header bar ──────────────────────
// All 9 panels (caption ×3, font ×3, audio ×2, media) share .cap-ov-panel /
// .cap-ov-head, so one generic handler covers all of them. Position resets
// to centered each time an overlay closes (see _ovResetPositions calls).
function _ovResetPositions() {
    document.querySelectorAll('.cap-ov-panel').forEach(p => {
        p.style.position = '';
        p.style.left     = '';
        p.style.top      = '';
        p.style.width    = '';
...
function _ovWireDraggable() {
    document.querySelectorAll('.cap-overlay .cap-ov-head').forEach(head => {
        if (head.dataset.dragWired) return;
        head.dataset.dragWired = '1';
        const panel = head.closest('.cap-ov-panel');
        if (!panel) return;
        ...
        const onDown = (e) => {
            if (e.target.closest('.cap-ov-close')) return;
            const rect = panel.getBoundingClientRect();
            const pt = e.touches ? e.touches[0] : e;
...
function _populateMediaOverlay() {
    ...
    // Thumbnail
    const fn = (sc.image_file || '').trim();
    const folder = (resolveMediaFolder(sc.image_folder) || IMG_FOLDER).replace(/\/?$/,'/');
...
// ── Upload ────────────────────────────────────────────────────────────────────
// ═════════════════════════════════════════════════════════════════════════════
// CHANGE MODEL / CHANGE BACKGROUND (tryon videos only) — both AJAX actions
// live in vizard_scriptgen_tryon.php (where the rest of the tryon
// generation pipeline already lives), so these fetch that file directly
// rather than location.href.
// ═════════════════════════════════════════════════════════════════════════════
const TRYON_AJAX_URL = 'vizard_scriptgen_tryon.php';

async function _tryonPost(payload) {
    const fd = new FormData();
    Object.entries(payload).forEach(([k, v]) => fd.append(k, v));
    const r = await fetch(TRYON_AJAX_URL, { method: 'POST', body: fd });
    return await r.json();
}

// Shared theme-grid loader/renderer — used by both overlays.
async function _renderThemeGrid(gridElId, onPick) {
    const grid = document.getElementById(gridElId);
    grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;font-size:12px;color:var(--muted);padding:14px 0;">Loading themes…</div>';
    try {
        const d = await _tryonPost({ ajax_action: 'get_themes' });
        const themes = d.success ? (d.themes || []) : [];
        if (!themes.length) {
            grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;font-size:12px;color:var(--muted);">No themes found.</div>';
            return;
        }
        grid.innerHTML = themes.map(t => `
            <div class="theme-pick-card" data-theme="${escAttr(t.theme_key)}" style="cursor:pointer;border-radius:10px;overflow:hidden;border:1.5px solid var(--border);">
                <div style="width:100%;aspect-ratio:9/16;overflow:hidden;background:var(--surface2);">
                    <img src="${escAttr(t.preview_image)}" style="width:100%;height:100%;object-fit:cover;display:block;">
                </div>
                <div style="font-size:10px;font-weight:700;text-align:center;padding:4px 2px;background:var(--surface);">${escHtmlVM(t.theme_name)}</div>
            </div>`).join('');
        grid.querySelectorAll('.theme-pick-card').forEach(card => {
            card.onclick = () => onPick(card.dataset.theme);
        });
    } catch (e) {
        grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;font-size:12px;color:#dc2626;">Failed to load themes.</div>';
    }
}
// Tiny local escaping helpers (kept self-contained — doesn't assume escHtml/esc
// from elsewhere in this file share the exact same signature).
function escAttr(s) { return String(s ?? '').replace(/"/g, '&quot;'); }
function escHtmlVM(s) { return String(s ?? '').replace(/[&<>]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;'}[c])); }

function _closeAllSubIconsAndOverlays() {
    document.getElementById('capSubIcons').style.display='none';
    document.getElementById('ibCaption')?.classList.remove('active');
    document.getElementById('fontSubIcons').style.display='none';
    document.getElementById('ibFont')?.classList.remove('active');
    document.getElementById('audioSubIcons').style.display='none';
    document.getElementById('ibAudio')?.classList.remove('active');
    closeCapOverlay(); closeFontOverlay(); closeAudioOverlay(); closeMediaOverlay();
    closeChangeModelOverlay(); closeChangeBgOverlay();
    _syncSubRail();
}
...
function onChangeModel() {
    const ov = document.getElementById('changeModelOverlay');
    if (ov.classList.contains('open')) { closeChangeModelOverlay(); return; }
    _closeAllSubIconsAndOverlays();
    document.getElementById('ibChangeModel')?.classList.add('active');
    document.getElementById('changeModelSceneNum').textContent = currentIndex + 1;
    document.getElementById('changeModelFormSection').style.display = '';
    document.getElementById('changeModelThemeSection').style.display = 'none';
    document.getElementById('changeModelStatus').textContent = '';
    document.getElementById('cmDescription').value = '';
    document.getElementById('cmPreset').value = '';
    const btn = document.getElementById('cmApplyBtn');
    btn.disabled = false; btn.textContent = 'Apply New Model (5 cr)';
    ov.classList.add('open');
    ov.onclick = e => { if (e.target === ov) closeChangeModelOverlay(); };
}
function closeChangeModelOverlay() {
    document.getElementById('changeModelOverlay')?.classList.remove('open');
    document.getElementById('ibChangeModel')?.classList.remove('active');
}
function _cmApplyPreset() {
    const sel = document.getElementById('cmPreset');
    if (sel.value) document.getElementById('cmDescription').value = sel.value;
}

async function submitChangeModel() {
    const description = document.getElementById('cmDescription').value.trim();
    if (!description) { alert('Model description is required — pick a quick preset or type your own'); return; }
    const sc = SCENES[currentIndex];
    const resolution = document.getElementById('cmResolution').value;

    // Ask about the background BEFORE calling anything, so a single
    // confirmed choice drives exactly one API call.
    const alsoChangeBg = confirm('Model updated. Do you want to change the background too?');

    if (!alsoChangeBg) {
        await _cmFinalize({ story_id: sc.id, model_description: description, resolution, change_background: 'false' });
        return;
    }

    // Show the theme grid inline — picking a card finalizes the call with
    // change_background=true + the chosen theme.
    document.getElementById('changeModelFormSection').style.display = 'none';
    document.getElementById('changeModelThemeSection').style.display = '';
    _renderThemeGrid('changeModelThemeGrid', async (themeKey) => {
        await _cmFinalize({ story_id: sc.id, model_description: description, resolution, change_background: 'true', theme_name: themeKey });
    });
}

async function _cmFinalize(payload) {
    const status = document.getElementById('changeModelStatus');
    status.textContent = '⏳ Generating new model…';
    try {
        const d = await _tryonPost({ ajax_action: 'change_scene_model', ...payload });
        if (!d.success) throw new Error(d.message || 'Failed');
        const sc = SCENES[currentIndex];
        sc.image_file = d.filename;
        await loadScene(currentIndex);
        if (typeof d.credits_balance === 'number') {
            USER_CREDIT = d.credits_balance;
            document.getElementById('creditBalanceDisplay').textContent = USER_CREDIT.toFixed(2);
        }
        status.textContent = '✅ Model updated';
        setTimeout(closeChangeModelOverlay, 700);
    } catch (e) {
...
// ── Change Background ───────────────────────────────────────────────────────
function onChangeBg() {
    const ov = document.getElementById('changeBgOverlay');
    if (ov.classList.contains('open')) { closeChangeBgOverlay(); return; }
    _closeAllSubIconsAndOverlays();
    document.getElementById('ibChangeBg')?.classList.add('active');
    document.getElementById('changeBgStatus').textContent = '';
    ov.classList.add('open');
    ov.onclick = e => { if (e.target === ov) closeChangeBgOverlay(); };
    _renderThemeGrid('changeBgThemeGrid', (themeKey) => submitChangeBackground(themeKey));
}
function closeChangeBgOverlay() {
    document.getElementById('changeBgOverlay')?.classList.remove('open');
    document.getElementById('ibChangeBg')?.classList.remove('active');
}

// themeKey === null → "shuffle": reuse the video's current theme, just pick
// different random photos from the same pool. Updates EVERY scene in this
// podcast, not just the one currently open — each scene keeps its own pose,
// only the background changes.
async function submitChangeBackground(themeKey) {
    const status = document.getElementById('changeBgStatus');
    status.textContent = '⏳ Updating background for all scenes…';
    try {
        const payload = { ajax_action: 'change_podcast_background', podcast_id: PODCAST_ID };
        if (themeKey) payload.theme_name = themeKey;
        const d = await _tryonPost(payload);
        if (!d.success) {
            if (d.upgrade_needed) throw new Error(d.message || 'Insufficient credits');
            throw new Error(d.message || 'Failed');
        }

        // Apply each updated scene's new filename to SCENES[], wherever
        // that scene currently sits in the array.
        const byId = {};
        (d.updated || []).forEach(u => { byId[String(u.story_id)] = u.filename; });
        SCENES.forEach(sc => { if (byId[String(sc.id)]) sc.image_file = byId[String(sc.id)]; });
        await loadScene(currentIndex);

        if (typeof d.credits_balance === 'number' && d.plan_type !== 'free_trial') {
            USER_CREDIT = d.credits_balance;
            document.getElementById('creditBalanceDisplay').textContent = USER_CREDIT.toFixed(2);
        }
        ...
        status.textContent = failCount
            ? `✅ Updated ${d.updated.length} scene(s) — ${failCount} failed`
            : `✅ Background updated for all ${d.updated.length} scene(s)`;
        setTimeout(closeChangeBgOverlay, 1000);
    } catch (e) {
        status.textContent = '✕ ' + e.message;
    }
}
...
async function handleMediaUpload(input) {
    ...
    // Tell server to store in user's personal folder (split by media type)
    const targetFolder = isVid ? USER_MEDIA_FOLDER_VIDEOS : USER_MEDIA_FOLDER_IMAGES;
    fd.append('target_folder', targetFolder.replace(/\/?$/,''));
    try {
        ...
        sc.image_folder = data.image_folder || targetFolder.replace(/\/?$/,'').replace(/^user_media\//,'');
        FILE_FOLDER[data.filename] = (resolveMediaFolder(sc.image_folder) || 'podcast_images') + '/';
        await loadScene(currentIndex);
        _populateMediaOverlay();
...
function mediaLibrary() {
    const sc = SCENES[currentIndex];
    // Default search query: podcast sub-category (e.g. "Bridal Wear") takes
    // priority since it reflects the whole show's niche; fall back to
    // scene-specific tags/hashtags, then the broader category/niche.
    let q = PODCAST_SUBCATEGORY || '';
    if (!q && sc.natural_language_tags) q = sc.natural_language_tags.trim().split('|')[0].trim();
    else if (!q && sc.hashtags)         q = sc.hashtags.trim().split(/\s+/)[0].replace(/^#/,'');
    if (!q) q = PODCAST_CATEGORY || PODCAST_NICHE || '';
...
function _updateLibCounts() {
    const im=document.getElementById('libCountImg'), v=document.getElementById('libCountVid');
    const mi=document.getElementById('libCountMineImg'), mv=document.getElementById('libCountMineVid');
    if(im) im.textContent = _libImgs.length;
    if(v)  v.textContent  = _libVids.length;
    if(mi) mi.textContent = _libMineImgs.length;
    if(mv) mv.textContent = _libMineVids.length;
    const st = document.getElementById('libSearchStatus');
...
async function _loadMyUploads(mediaType) {
    // mediaType: 'image' or 'video'
    const grid=document.getElementById('libGrid');
    const folderConst = mediaType==='video' ? USER_MEDIA_FOLDER_VIDEOS : USER_MEDIA_FOLDER_IMAGES;
    const label = mediaType==='video' ? 'User Videos' : 'User Images';
    if(grid) grid.innerHTML=`<div style="grid-column:1/-1;text-align:center;padding:40px;color:var(--muted);">Loading your ${label.toLowerCase()}…</div>`;
    try {
        const fd=new FormData();
        fd.append('ajax_action','get_user_media');
        fd.append('media_type', mediaType);                                  // 'image' | 'video'
        fd.append('subfolder', mediaType==='video' ? 'podcast_videos' : 'podcast_images');
        const r=await fetch(location.href,{method:'POST',body:fd});
        const data=await r.json();
        const target = mediaType==='video' ? '_libMineVids' : '_libMineImgs';
        if(!data.has_folder||!data.files||!data.files.length){
            ...
            if(grid) grid.innerHTML=`<div style="grid-column:1/-1;text-align:center;padding:60px 20px;color:var(--muted);">
                ...
                <div style="font-weight:600;margin-bottom:6px;">No ${label.toLowerCase()} yet</div>
                <div style="font-size:11px;line-height:1.6;">Use <strong>📤 Upload</strong> to add files to your folder.<br>
                <span style="opacity:.55;font-size:10px;">${folderConst}</span></div></div>`;
            _updateLibCounts();
            return;
        }
        const folderUsed = (data.folder || folderConst).replace(/\/?$/,'/');
        if (mediaType==='video') window._userMediaFolderVideos = folderUsed;
        else                     window._userMediaFolderImages = folderUsed;
        window[target]=data.files.map(f=>({filename:f.filename,media_type:mediaType,nl_tags:'',score:0,thumbnail:f.thumbnail||'',is_user_media:true,matched_segment:'',matched_line:''}));
        _updateLibCounts(); _renderLibGrid();
    } catch(e) {
...
async function performLibSearch() {
    ...
    if (_libTab==='mine_video' || _libTab==='mine_image') {
        // Personal media has no NL search index — filter the loaded list by filename.
        const mediaType = _libTab==='mine_video' ? 'video' : 'image';
        await _loadMyUploads(mediaType);
        if (q) {
            const ql = q.toLowerCase();
            if (_libTab==='mine_video') _libMineVids = _libMineVids.filter(f=>f.filename.toLowerCase().includes(ql));
            else                        _libMineImgs = _libMineImgs.filter(f=>f.filename.toLowerCase().includes(ql));
            _updateLibCounts(); _renderLibGrid();
        }
        return;
    }
...
function setLibTab(type) {
    _libTab=type;
    ['video','image','mine_video','mine_image'].forEach(t=>{
        const ids={video:'libTabVid',image:'libTabImg',mine_video:'libTabMineVid',mine_image:'libTabMineImg'};
        document.getElementById(ids[t])?.classList.toggle('active',t===type);
    });
    if(type==='mine_video'){ _loadMyUploads('video'); return; }
    if(type==='mine_image'){ _loadMyUploads('image'); return; }
    const q=(document.getElementById('libSearch')?.value||'').trim();
...
function _renderLibGrid() {
    ...
    const files = _libTab==='video'       ? _libVids
                ...
                : _libTab==='mine_video'  ? _libMineVids
                : _libTab==='mine_image'  ? _libMineImgs
                : [];

    if (!files.length) {
        const isMine = _libTab==='mine_video' || _libTab==='mine_image';
        const icon = isMine ? '🗂️' : _libTab==='video' ? '🎬' : '🖼️';
        const labels = {video:'videos',image:'images',mine_video:'user videos',mine_image:'user images'};
        const msg  = isMine ? `No ${labels[_libTab]} — use 📤 Upload`
                   : `No ${labels[_libTab]} found`;
    ...
    const cards = files.map((f, i) => {
        ...
        const folder = f.is_user_media
            ? (f.media_type==='video' ? (window._userMediaFolderVideos || USER_MEDIA_FOLDER_VIDEOS)
                                       : (window._userMediaFolderImages || USER_MEDIA_FOLDER_IMAGES))
            : getFileFolder(f.filename);
...
async function useLibraryFile(){
    ...
    // Determine folder to save — Mine tab keeps the full path (incl. 'user_media/'
    // prefix) since getSceneMediaFolder() uses image_folder verbatim when
    // rendering; a stripped path would 404.
    let folderToSave;
    if(_libTab==='mine_video'||_libTab==='mine_image'||(folder&&folder.indexOf('user_media')!==-1)){
        folderToSave=(folder||(isVidFile?USER_MEDIA_FOLDER_VIDEOS:USER_MEDIA_FOLDER_IMAGES)).replace(/\/?$/,'');
    } else {
    ...
    try {
        ...
        FILE_FOLDER[filename]=(resolveMediaFolder(sc.image_folder) || 'podcast_images')+'/';
        closeLibraryModal();
        await loadScene(currentIndex);
        _populateMediaOverlay();
...
canvas.addEventListener('mouseup', () => {
    // Route through capFieldChanged so position propagates to all scenes
    // and saves to hdb_user_settings for main/header/footer/logo captions
    ...
    if (_resize.active && _resize.capId) {
        ...
        if (cap) {
            ['width','rotation','position_x','position_y'].forEach(f => {
                capFieldChanged(f, Math.round(parseFloat(cap[f])||0));
            });
        }
    }
    _drag.active = false;
...
<!-- ══ GENERATE ERROR PANEL (replaces unselectable alert()) ══ -->
<div id="generateErrorPanel" style="display:none;position:fixed;inset:0;background:rgba(15,23,42,.7);z-index:99996;align-items:center;justify-content:center;padding:16px;">
    <div style="background:#fff;border-radius:14px;max-width:560px;width:100%;max-height:80vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,.4);">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 18px;background:#fef2f2;border-bottom:1px solid #fecaca;flex-shrink:0;">
            <span style="font-weight:800;color:#991b1b;">❌ Generation failed</span>
            <button onclick="document.getElementById('generateErrorPanel').style.display='none'"
                style="background:#fee2e2;border:none;border-radius:50%;width:28px;height:28px;cursor:pointer;color:#991b1b;font-weight:700;">✕</button>
        </div>
        <div id="generateErrorBody" style="padding:16px 18px;overflow-y:auto;font-size:12px;line-height:1.6;color:#1e293b;white-space:pre-wrap;word-break:break-word;font-family:monospace;flex:1;user-select:text;"></div>
        <div style="padding:10px 18px;border-top:1px solid #e2e8f0;flex-shrink:0;display:flex;gap:8px;justify-content:flex-end;">
            <button onclick="_copyGenerateError()" style="padding:7px 14px;border-radius:8px;border:1.5px solid #cbd5e1;background:#f8fafc;font-size:12px;font-weight:700;cursor:pointer;">📋 Copy</button>
            <button onclick="document.getElementById('generateErrorPanel').style.display='none'" style="padding:7px 14px;border-radius:8px;border:none;background:#0f2a44;color:#fff;font-size:12px;font-weight:700;cursor:pointer;">Close</button>
        </div>
    </div>
</div>
<script>
function showGenerateError(data) {
    let detail = '❌ ' + (data.error || 'Unknown error') + '\n';
    if (data.debug_log_url) detail += '\nFull log: ' + data.debug_log_url + '\n';
    if (data.skip_log && data.skip_log.length) detail += '\nWhy scenes were skipped:\n' + data.skip_log.join('\n') + '\n';
    if (data.checked && data.checked.length)   detail += '\nPaths checked:\n' + data.checked.join('\n') + '\n';
    if (data.log) detail += '\nFFmpeg log:\n' + data.log + '\n';
    const body = document.getElementById('generateErrorBody');
    if (body) body.textContent = detail;
    const panel = document.getElementById('generateErrorPanel');
    if (panel) panel.style.display = 'flex';
    if (data.debug_log_url) { try { window.open(data.debug_log_url, '_blank'); } catch(e) {} }
}
function _copyGenerateError() {
    const body = document.getElementById('generateErrorBody');
    if (!body) return;
...
<!-- ── Simple "Video Ready" modal — replaces the full Publish Video modal
     for now (that one's markup/JS below is left intact, just unused, in
     case it comes back later). Three actions only: Download, Schedule
     (hands off to vizard_scheduler.php), Review (same player as the
     existing Review button). ──────────────────────────────────────────── -->
<div class="bpm-overlay" id="vidReadyOverlay">
  <div class="bpm-modal" style="max-width:380px;">
    <div class="bpm-head">
      <div class="bpm-head-left">
        <div class="bpm-head-icon">🎬</div>
        <div>
          <div class="bpm-head-title">Video Ready!</div>
          <div class="bpm-head-sub" id="vidReadySub">Your video has been generated</div>
        </div>
      </div>
      <button class="bpm-close" onclick="closeVidReadyModal()">✕</button>
    </div>
    <div class="bpm-saved" id="vidReadyFilenameDisplay">
      <div class="bpm-saved-dot"></div>
      <span>Video ready</span>
    </div>
    <div style="padding:18px 20px;display:flex;flex-direction:column;gap:10px;">
      <button class="bpm-dl-btn" style="grid-column:unset;width:100%;" onclick="vidReadyDownload()">⬇ Download MP4</button>
      <button class="bpm-btn-sched" style="grid-column:unset;width:100%;" onclick="vidReadySchedule()">🗓 Schedule</button>
      <button class="btn-action btn-review" style="width:100%;justify-content:center;" onclick="vidReadyReview()">👁 Review</button>
    </div>
  </div>
</div>
