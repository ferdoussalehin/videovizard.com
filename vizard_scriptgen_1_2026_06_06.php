...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'deduct_video_credit') {
    ...
    $urow = mysqli_fetch_assoc(mysqli_query($conn, "SELECT role, team_lead_id FROM hdb_users WHERE id=$admin_id LIMIT 1"));
    $role = $urow['role'] ?? 'Team Lead';
    ...
    if (function_exists('deduct_user_credit')) {
        ...
        $row = mysqli_fetch_assoc(mysqli_query($conn, "SELECT credit_balance FROM hdb_users WHERE id=$target_id LIMIT 1"));
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'get_video_quota') {
    if (ob_get_length()) ob_clean(); header('Content-Type: application/json');
    $urow = mysqli_fetch_assoc(mysqli_query($conn,"SELECT plan_type,role,team_lead_id,credit_balance FROM hdb_users WHERE id=$admin_id LIMIT 1"));
    $pt   = $urow['plan_type']     ?? 'free_trial';
    $role = $urow['role']          ?? 'Team Lead';
    $tl   = (int)($urow['team_lead_id'] ?? 0);
    if ($role === 'Team Member' && $tl > 0) {
        $crow = mysqli_fetch_assoc(mysqli_query($conn,"SELECT credit_balance FROM hdb_users WHERE id=$tl LIMIT 1"));
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'get_ai_video_suggestions') {
    ...
    if ($httpCode == 200) {
        ...
        if (is_array($items) && count($items) > 0) echo json_encode(['success'=>true,'suggestions'=>array_slice($items,0,10)]);
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'save_video_idea') {
    ...
    if ($store_as_common && $is_ai) {
        $exists = mysqli_fetch_assoc(mysqli_query($conn,"SELECT id FROM hdb_user_video_ideas WHERE admin_id=0 AND niche_name='$niche_name' AND category_name='$category_name' AND video_idea='$video_idea' LIMIT 1"));
        ...
        $exists = mysqli_fetch_assoc(mysqli_query($conn,"SELECT id FROM hdb_user_video_ideas WHERE admin_id=$owner_id AND company_id=$co_id AND niche_name='$niche_name' AND category_name='$category_name' AND video_idea='$video_idea' LIMIT 1"));
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'get_master_niches') {
    ...
    $t = mysqli_fetch_assoc(mysqli_query($conn,
        "SELECT COUNT(*) cnt FROM hdb_master_niches WHERE master_industry_id IN ($ids_sql)"))['cnt'] ?? 0;
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'save_niche') {
    ...
    if ($store_as_common && $is_ai) {
        $e=mysqli_fetch_assoc(mysqli_query($conn,"SELECT id FROM hdb_user_niches WHERE admin_id=0 AND niche_name='$niche_name' LIMIT 1"));
        ...
        $e=mysqli_fetch_assoc(mysqli_query($conn,"SELECT id FROM hdb_user_niches WHERE admin_id=$owner_id AND company_id=$co_id AND niche_name='$niche_name' LIMIT 1"));
        ...
        $e=mysqli_fetch_assoc(mysqli_query($conn,"SELECT id FROM hdb_user_niches WHERE admin_id=$admin_id AND niche_name='$niche_name' LIMIT 1"));
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'save_category') {
    ...
    if ($nq && $nr=mysqli_fetch_assoc($nq)) $niche_id=(int)$nr['id'];
    if ($company_type !== 'internal') {
        $e=mysqli_fetch_assoc(mysqli_query($conn,"SELECT id FROM hdb_user_categories WHERE admin_id=$owner_id AND company_id=$co_id AND niche_name='$niche_name' AND category_name='$category_name' LIMIT 1"));
        ...
        $e=mysqli_fetch_assoc(mysqli_query($conn,"SELECT id FROM hdb_user_categories WHERE admin_id=0 AND niche_name='$niche_name' AND category_name='$category_name' LIMIT 1"));
        ...
        $e=mysqli_fetch_assoc(mysqli_query($conn,"SELECT id FROM hdb_user_categories WHERE admin_id=$admin_id AND niche_name='$niche_name' AND category_name='$category_name' LIMIT 1"));
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'save_angle') {
    ...
    $e = mysqli_fetch_assoc(mysqli_query($conn,"SELECT id FROM hdb_user_angles WHERE admin_id=$target_admin AND angle_name='$angle_name' LIMIT 1"));
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'save_user_cta') {
    ...
    $e=mysqli_fetch_assoc(mysqli_query($conn,"SELECT id FROM hdb_user_cta WHERE company_id=$co_id AND cta_text='$cta_text' LIMIT 1"));
...
// ── Init captions from user settings ─────────────────────────────────────────
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'init_captions_from_settings') {
    ...
    $pod_row = mysqli_fetch_assoc(mysqli_query($conn, "SELECT video_type FROM hdb_podcasts WHERE id=$pid LIMIT 1"));
...
function vv_splitAndTag(string $raw, string $reel_type = ''): string {
    ...
    if (count($scenes) > 1) {
        ...
        return vv_mergeShortScenes($tagged, 15, 35);
    }
    $scenes = preg_split('/\r?\n/', $raw);
    $scenes = array_values(array_filter(array_map('trim', $scenes)));
    if (count($scenes) > 1) {
...
function vv_buildPrompt(array $d): string {
    ...
    $content_goals   = trim($d['content_goals']   ?? $d['objective'] ?? 'Educate');
    ...
    // Goal style block
    $goal_styles = [
        'Promote'     => ['writing_style'=>'Promotional and desire-building. Make the viewer want to visit, buy, or order.','hook_style'=>'Lead with the product, result, or experience — create immediate desire or FOMO.','structure'=>'Hook → showcase the product/service → highlight what makes it special → CTA.','cta_tone'=>'Direct: visit us, order now, book today.','avoid'=>'Generic tips, anything not tied to the business.'],
        'Educate'     => ['writing_style'=>'Clear, authoritative, genuinely useful. Teach one specific thing.','hook_style'=>'Lead with a surprising fact, a common mistake, or a question.','structure'=>'Hook → explain step by step → concrete example → actionable takeaway → CTA.','cta_tone'=>'Value-driven: follow for more tips, save this for later.','avoid'=>'Vague generalisations, promotional language.'],
        'Build Trust' => ['writing_style'=>'Honest, transparent, human. Show the real side of the business.','hook_style'=>'Open with an honest admission or behind-the-scenes reveal.','structure'=>'Hook → show real process or people → explain the why → soft CTA.','cta_tone'=>'Soft: come visit us, follow along for more.','avoid'=>'Hard sell, exaggerated claims, polished marketing speak.'],
        'Entertain'   => ['writing_style'=>'Fun, energetic, shareable. Prioritise the laugh or surprise.','hook_style'=>'Open with something unexpected, funny, or visually irresistible.','structure'=>'Hook → build the moment → deliver the payoff → light CTA.','cta_tone'=>'Light: follow for more, tag a friend.','avoid'=>'Dry information, corporate tone, lectures.'],
        'Inform'      => ['writing_style'=>'Newsy, clear, timely. Share something relevant happening now.','hook_style'=>'Lead with the news itself — what\'s new, changed, or coming.','structure'=>'Announce the news → key details → CTA.','cta_tone'=>'Informative: find out more, check the link.','avoid'=>'Storytelling for its own sake, deep tutorials.'],
        'Inspire'     => ['writing_style'=>'Emotional, warm, story-driven. Connect on a human level.','hook_style'=>'Open with an emotional truth or a personal moment.','structure'=>'Hook → tell the story → reveal the lesson → CTA.','cta_tone'=>'Warm: follow our journey, join our community.','avoid'=>'Cold facts, promotional language, anything transactional.'],
    ];
    $gs = $goal_styles[$content_goals] ?? $goal_styles['Educate'];
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'generate_script') {
    ...
    $d = [
        ...
        'content_goals' => trim($_POST['content_goals'] ?? 'Educate'),
    ...
    if (strpos($rt_lower, 'podcast') === false && strpos($rt_lower, 'talking head') === false) {
        $script = vv_mergeShortScenes($script, 15, 35);
    }
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'save_company_industry') {
    ...
    if (!$co_id) {
        ...
        $fb = mysqli_fetch_assoc(mysqli_query($conn, "SELECT id FROM hdb_companies WHERE admin_id=$admin_id ORDER BY id ASC LIMIT 1"));
...
<div class="page-wrap">
  ...
  <div id="modeCampaign" style="display:none;width:100%;max-width:600px;">
    <div class="wiz-card">
      <div class="wiz-header"><div><h1>📅 Campaign — Coming Soon</h1></div></div>
      <div class="wiz-body">
        <button class="back-to-menu" onclick="goToMenu()">← All options</button>
        <div class="todo-banner" style="margin-top:20px;">
          <h3>🚧 Campaign mode not yet wired in this version</h3>
          <p>This will be added step by step. Go back to use the wizard for now.</p>
        </div>
      </div>
    </div>
  </div>
  ...
  <div id="modeContent" style="display:none;width:100%;max-width:600px;">
    <div class="wiz-card">
      <div class="wiz-header"><div><h1>📄 I Have Content — Coming Soon</h1></div></div>
      <div class="wiz-body">
        <button class="back-to-menu" onclick="goToMenu()">← All options</button>
        <div class="todo-banner" style="margin-top:20px;">
          <h3>🚧 Content mode not yet wired in this version</h3>
          <p>This will be added step by step. Go back to use the wizard for now.</p>
        </div>
      </div>
    </div>
...
function renderBusinessBar() {
    ...
    ['business-bar-pills','ideas-business-pills'].forEach(function(id) {
...
function renderSettingsBar() {
    ...
    ['settings-bar-pills','ideas-video-pills'].forEach(function(id) {
        var el = document.getElementById(id);
        if (el) el.innerHTML = html;
    });
}
...
function selectMode(mode) {
    ...
    if (mode === 'wizard') {
        ...
        if (coIndustry.group && coIndustry.subgroup && coIndustry.niche) {
            // Profile complete — show ideas box directly
            document.getElementById('modeWizard').style.display = 'none';
            showIdeasBox();
        } else {
            // Profile not set — show wizard with both bars, open business modal
            ans = {}; cur = 0;
            render();
...
function _renderScript(scenes, wordCount, reelType) {
    ...
    var pills = [dur + 's', settings.language||'English', reelLabel,
                 settings.tone||'Friendly', scenes.length + ' lines',
...
async function renderVoiceSelect(s) {
    ...
    function buildOpts(gender) {
        ...
        var azure  = filtered.filter(function(v){ return !(v.voice_id||'').startsWith('openai:'); });
        var html = '';
        if (openai.length) {
            html += '<optgroup label="OpenAI">';
            openai.forEach(function(v){
                var sel = ans.voice_id === v.voice_id ? ' selected' : '';
                html += '<option value="' + esc(v.voice_id) + '"' + sel + ' data-sample="' + esc(v.sample_voice||'') + '">' + escHtml(v.voice_name) + '</option>';
            });
            html += '</optgroup>';
        }
        if (azure.length) {
            html += '<optgroup label="Azure' + (IS_FREE_TRIAL ? ' 🔒 Subscribers Only' : '') + '">';
            azure.forEach(function(v){
                var dis = IS_FREE_TRIAL ? ' disabled' : '';
                var sel = ans.voice_id === v.voice_id ? ' selected' : '';
                html += '<option value="' + esc(v.voice_id) + '"' + sel + dis + ' data-sample="' + esc(v.sample_voice||'') + '">' + escHtml(v.voice_name) + (IS_FREE_TRIAL ? ' 🔒' : '') + '</option>';
            });
            html += '</optgroup>';
        }
...
function wizFilterVoices(gender, btn) {
    ...
    if (openai.length) {
        html += '<optgroup label="OpenAI">';
        openai.forEach(function(v){
            html += '<option value="' + esc(v.voice_id) + '" data-sample="' + esc(v.sample_voice||'') + '">' + escHtml(v.voice_name) + '</option>';
        });
        html += '</optgroup>';
    }
    if (azure.length) {
        var azLabel = 'Azure' + (IS_FREE_TRIAL ? ' 🔒 Subscribers Only' : '');
        html += '<optgroup label="' + azLabel + '">';
        azure.forEach(function(v){
            var dis = IS_FREE_TRIAL ? ' disabled' : '';
            html += '<option value="' + esc(v.voice_id) + '"' + dis + ' data-sample="' + esc(v.sample_voice||'') + '">' + escHtml(v.voice_name) + (IS_FREE_TRIAL ? ' 🔒' : '') + '</option>';
        });
        html += '</optgroup>';
    }
    var sel = document.getElementById('wiz-voice-select');
...
function wizPlaySample() {
    ...
    if (opt.disabled) { showToast('🔒 Azure voices require a subscription'); return; }
    // Stop if already playing
    if (_wizVoiceAudio) {
        _wizVoiceAudio.pause(); _wizVoiceAudio = null;
        btn.textContent = '▶ Play Sample'; btn.classList.remove('playing');
        if (btn.dataset.playing === '1') { btn.dataset.playing = '0'; return; }
    }
    var sampleFile = opt.getAttribute('data-sample') || '';
    ...
    _wizVoiceAudio = new Audio('podcast_audios/' + sampleFile + '?t=' + Date.now());
    var reset = function() { _wizVoiceAudio = null; btn.textContent = '▶ Play Sample'; btn.classList.remove('playing'); btn.dataset.playing = '0'; };
    _wizVoiceAudio.onended = reset;
    _wizVoiceAudio.onerror = function() { reset(); showToast('Could not play sample'); };
    _wizVoiceAudio.play().catch(function() { reset(); showToast('Could not play sample'); });
}
...
function showFinalStep() {
    ...
    document.getElementById('step-body').innerHTML =
        ...
        + row('Content Goal', settings.content_goals || 'Educate',        '')
