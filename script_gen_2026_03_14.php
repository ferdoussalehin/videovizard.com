...
async function processIdeaContent(retryCount = 0) {
    ...
    let topic = '';
    if (topicSelect && topicSelect.value && topicSelect.value !== '__manual__' && topicSelect.value !== '') {
...
async function assignImagesToAllScenes(podcastId) {
    ...
    let duplicatesRejected = 0; // Track rejected duplicates
    
    // TRACK USED MEDIA TO PREVENT DUPLICATES
    const usedMedia = new Set(); // Store filenames that have been assigned
    ...
    for (let i = 0; i < scenes.length; i++) {
        ...
        if (results && results.length > 0) {
            // Log all results with their status
            L(`📊 Found ${results.length} total matches:`, 'info');
            results.forEach((item, idx) => {
                const status = usedMedia.has(item.filename) ? '🔴 DUPLICATE (already used)' : '🟢 AVAILABLE';
                L(`   ${idx+1}. ${item.filename} (${item.type}) - ${status}`, 'info');
            });
            
            // FILTER OUT ALREADY USED MEDIA
            const availableResults = results.filter(item => !usedMedia.has(item.filename));
            const duplicateCount = results.length - availableResults.length;
            
            if (duplicateCount > 0) {
                duplicatesRejected += duplicateCount;
                L(`🚫 Rejected ${duplicateCount} duplicate media file(s) that were used in previous scenes`, 'warning');
            }
            
            if (availableResults.length === 0) {
                L(`⚠️ ALL ${results.length} matches are already used in previous scenes - no media available`, 'warning');
                L(`🎨 Falling back to AI generation...`, 'info');
                ...
                if (generated && generated.success) {
                    ...
                    usedMedia.add(generated.filename); // Track this new generated image
            ...
            L(`✅ ${availableResults.length} media files available (not used yet)`, 'success');
            
            const videos = availableResults.filter(r => r.type === 'video');
            const images = availableResults.filter(r => r.type === 'image');
            
            L(`   Available Videos: ${videos.length}, Available Images: ${images.length}`, 'info');
            
            // RANDOM SELECTION with preference for videos (70% video, 30% image)
            ...
            if (videos.length > 0) {
                ...
                if (randomValue < 0.7 || images.length === 0) {
                    ...
                    videosAssigned++;
                    L(`🎬 Randomly selected video ${randomIndex + 1}/${videos.length} from available`, 'success');
                    L(`   Selected: ${selectedItem.filename}`, 'info');
                } else if (images.length > 0) {
                    // Pick a random image from available ones
                    const randomIndex = Math.floor(Math.random() * images.length);
                    selectedItem = images[randomIndex];
                    imagesAssigned++;
                    L(`🖼️ Randomly selected image ${randomIndex + 1}/${images.length} from available`, 'success');
                    L(`   Selected: ${selectedItem.filename}`, 'info');
                }
            } else if (images.length > 0) {
            ...
            if (selectedItem) {
                ...
                usedMedia.add(assignedImage); // Mark as used
                
                const assigned = await assignImageToScene(scene.id, assignedImage);
                if (assigned) {
                    L(`✅ Assigned to scene ${sceneNumber} (${selectedItem.type})`, 'success');
                } else {
